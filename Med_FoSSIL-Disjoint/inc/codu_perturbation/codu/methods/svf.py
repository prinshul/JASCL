...
class SVD_Conv2d(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size,
                 ...
                 padding_mode='zeros', device='cuda', dtype=None,
                 rank=1):
...
class SVD_Linear(nn.Module):
    ...
    def __init__(self, in_features, out_features, bias, device='cuda', dtype=None, rank=1):
...
class SVD_Conv3d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride, padding, dilation, groups, bias, padding_mode='zeros', device='cuda', dtype=None, rank=1):
        ...
        self.vector_S = nn.Parameter(torch.ones((1, 1, rank, 1, 1), **factory_kwargs))
        ## torch.ones((1, rank, 1, 1, 1) replaces torch.empty
            ...
            if len(model._modules[sub_module_name]._modules) > 0:
                ...
                if type(getattr(model, sub_module_name)) in full2low_mapping and \
                    ...
                    if old_type == nn.Conv2d :
                        ...
                        #print(param_list[0].shape,param_list[1].shape,param_list[2].shape)
...
class DatafreeSVD(object):
    ...
    def __init__(self, model, global_rank_ratio=1.0,
        ...
        model_data_parallel = True if str(model_dict_key).startswith('module') else False
        self.model_cpu = self.model.module.to("cpu") if model_data_parallel else self.model.to("cpu")
        #self.model_cpu = self.model.module.cuda() if model_data_parallel else self.model.cuda()
        ...
        for m_name, m in self.model_named_modules:
            if type(m) in registered_param_op and m_name not in self.excluded_layers:
                ...
                #print(m_name, m,"tensor_shape",tensor_shape)
            ...
            if m_name in self.layer_rank.keys():
                ...
                if len(tensor_shape) == 1:
                    ...
                    self.param_lowrank_decomp_dict[m_name] = [
                        ...
                        S.reshape(1, 1, self.layer_rank[m_name], 1, 1)    
                    ]
