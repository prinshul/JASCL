...
class IDD(Dataset):
    ...
    def __init__(self, root, files, co_transform=None, subset='train'):
        self.images_root = os.path.join(root,subset,'images/')
        self.labels_root = os.path.join(root,subset,'labels/')
        ...
        self.filenames = []
        for af in files:
            self.filenames.append(af.split("/")[-1].replace("gtFine_labellevel3Ids","leftImg8bit"))
        ...
        self.filenamesGt = []
        for af in files:
            self.filenamesGt.append(af.split("/")[-1])
...
class BDD100k(Dataset):
    ...
    def __init__(self, root,files,co_transform=None,subset='train',out_files=False):
        self.images_root = os.path.join(root,subset,'images/')
        self.labels_root = os.path.join(root,subset,'labels/')
        ...
        #print("Images root path : ",self.images_root)
        #print("Labels root path : ",self.labels_root)
        #self.filenames = [image_basename(f) for f in os.listdir(self.images_root) if is_image(f)]
        ...
        self.filenames = []
        for af in files:
            self.filenames.append(af[:-3]+"jpg")
        #self.filenames.sort()
        #print("Total images : ",len(self.filenames))

        #[os.path.join(dp, f) for dp, dn, fn in os.walk(os.path.expanduser(".")) for f in fn]
        #self.filenamesGt = [image_basename(f) for f in os.listdir(self.labels_root) if is_image(f)]
        self.filenamesGt = files
        #self.filenamesGt.sort()
