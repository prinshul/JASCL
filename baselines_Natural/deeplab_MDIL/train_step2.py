...
def train(args, model, model_old, start_epoch, best_acc):
    ...
    weight_IDD = torch.tensor([3.6525147,8.799815,4.781908,10.034828,9.556787,
                 8.315293,8.163474,9.246903,6.0067043,9.606205,
                 10.787631,9.862216,10.446818,10.891895,9.891895,0])
    ...
    if data_name == 'IDD':
        print('taking IDD')
        dataset_train = IDD(IDD_datadir,train_gt_files, co_transform, 'train')
        dataset_val = IDD(IDD_datadir,val_gt_files, co_transform_val, 'train')
    ...
    ###################Previous step validation dataloader #####################
    ############################################################################
    
    '''
    weight_BDD = torch.tensor([0.1000, 0.4896, 0.1000, 1.9219, 0.9045, 5.4740, 2.8373, 4.2810, 0.1196,
        1.0000, 0.0000])
    weight_BDD[NUM_CLASSES_OLD - 1] = 0
    
    
    if args.dataset_old == 'BDD_step1':
        print('\n\nloading BDD_step1 as validation dataset, (old - step 1)')
        
        co_transform_val_old = MyCoTransform(augment=False,step_train=False, height=args.height, width=args.width)  # 1024)
        
        with open(shots_datadir+'gt_val_step1.pkl', 'rb') as file: 
            gt_val_step1=pickle.load(file)
        
        dataset_step1_val = BDD100k(BDD_datadir,gt_val_step1,co_transform_val_old,'train')
        
        loader_val_old = DataLoader(dataset_step1_val, num_workers=args.num_workers,
                            batch_size=args.batch_size, shuffle=False)
        
    
    if args.cuda:
        class_weights_old = weight_BDD.to(device)
        class_weights_old = torch.clamp(class_weights_old, 0.1, 10.0)
    
    print("Class weights Step 1",class_weights_old)
    
    print("Total files in val  Step 1",len(gt_val_step1))
    print("Total length of val loader Step 1",len(loader_val_old))
    ...
    #criterion_old = nn.CrossEntropyLoss(ignore_index=NUM_CLASSES_OLD - 1, weight=class_weights_old)
...
def test(args, model):
    ...
    test_loader_step2 = DataLoader(IDD_test_step2, num_workers=args.num_workers,
                        batch_size=args.batch_size, shuffle=False)
    
    print("Total test files in Step 2 testloader : ",len(IDD_test_step2))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', default="IDD")
    ...
    parser.add_argument('--num-classes', type=int, nargs="+", help='pass list with number of classes',
                        required=False, default=[11,16])  # send [20, 20] in IL-step2 (BDD), [20, 20, 27] in IL-step3 (IDD)
