...
class DeepLab(nn.Module):
    ...
    def __init__(self, backbone, current_task, num_classes):
        super().__init__()
        if not isinstance(backbone, BackboneModule):
            raise RuntimeError('Backbone must extend model.backbone.BackboneModue')

        num_all_classes = [11,16,19]
        self.task = current_task
        self.backbone = backbone
        self.aspp = DeepLab.ASPP(in_channels=backbone.out_channels,
                                 output_stride=backbone.output_stride)
        self.decoder = nn.ModuleList([DeepLab.Decoder(low_in_channels=backbone.low_out_channels,
                                       num_classes=num_classes) for i in range(self.task+1)])

        self._init_weights()

    def forward(self, x_in, task):
        x, x_low = self.backbone(x_in)
        x = self.aspp(x)
        ## task wise decoder
        logits = self.decoder[self.task].forward(x, x_low)
