...
def step_more_with_replay(opts,task,logger,rank,device):
    ...
    if exemp_dst is None:
        print("Exemplar set is not selected")
    else:
        print("The length of exemplar set is: {}".format(len(exemp_dst)))
    print("\n\nThe length of train set is: {}".format(len(train_dst)))
    ...
    print("Train loader checking of Concat loader")
    for ci_batch, csampled_batch in enumerate(train_loader):
        print("Patient ids", csampled_batch['case_name'])
    ...
    if task.step > 0:
        ...
        print("Base ckpt is ", step_ckpt['path'])
    ...
    print("Cur epoch and Epochs ",cur_epoch, max_epoch)
    ...
    while cur_epoch <= max_epoch and not opts.test:
        ...
        start = time.time()
        if opts.sample_exmp:
            print("Using concat loader")
            class_dice_train, train_dice = model.train(optimizer, dice_weight, ce_weight, cur_epoch=cur_epoch,
            train_loader=concat_loader,metrics=train_metrics, print_int=opts.print_interval,
                                     n_iter=train_iterations, snapshot_path = snapshot_path)
        else:
            print("Using train loader")
            class_dice_train, train_dice = model.train(optimizer, dice_weight, ce_weight, cur_epoch=cur_epoch,
            train_loader=train_loader,metrics=train_metrics, print_int=opts.print_interval,
                                     n_iter=train_iterations, snapshot_path = snapshot_path)
        # train_score = train_metrics.get_results()
        end = time.time()
