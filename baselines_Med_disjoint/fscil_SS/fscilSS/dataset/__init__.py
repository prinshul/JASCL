...
def get_dataset(opts, task, train=True):
    ...
    if train:
        if opts.cross_val:
            ...
            exemp_dst = None #dataset(root=opts.data_root, task=task, base_dir=opts.train_root_path, train="exemplar",
                            #  num_classes=opts.num_classes,transform=transforms.Compose(
                            #   [RandomGenerator(output_size=opts.img_size, mode = 'train')]))
            
            if opts.step==1 and opts.sample_exmp==True:
                print("Using step 1 labeled and unlabeled samples")
                exemp_dst = dataset(root=opts.data_root, task=task, base_dir=opts.train_root_path, train="exemplar1", 
                                num_classes=opts.num_classes,
                                   transform=transforms.Compose([RandomGenerator(output_size=opts.img_size, mode = 'test')]))
                                   
            if opts.step==2 and opts.sample_exmp==True:
                print("Using step 2 labeled and unlabeled samples")
                exemp_dst = dataset(root=opts.data_root, task=task, base_dir=opts.train_root_path, train="exemplar2", 
                                num_classes=opts.num_classes,
                                   transform=transforms.Compose([RandomGenerator(output_size=opts.img_size, mode = 'test')]))
