...
class Trainer:
    ...
    def save_labels_step(self,img,img_path, save_path, name):
        device=self.device
        imgd = sitk.GetImageFromArray(img.detach().cpu().numpy())
        sitk.WriteImage(imgd,img_path+name)
        print("Image saved = ", name)
        
        img = img.unsqueeze(0)
        img = img.unsqueeze(0)
        out, fout = self.model(img) #.unsqueeze(0)) #, use_classifier=False)
        #print("Model out, embed, enc", out.shape, fout.shape)
        del fout
        out = out.squeeze(0)
        
        label_mask = np.zeros((128,160,96))
        soft_mask=torch.softmax(out, dim=0).detach().cpu().numpy()
        del out
        soft_mask[soft_mask<0.5]=0
        soft_mask[soft_mask>=0.5]=1
        
        print(label_mask.shape, soft_mask.shape)
        for i in range(1,self.opts.num_classes):
            label_mask=label_mask + i*soft_mask[i]
        print(np.unique(label_mask))
        del soft_mask
        pred = sitk.GetImageFromArray(label_mask) #.detach().cpu().numpy())
        sitk.WriteImage(pred,save_path+name)
        
        print("Label saved for ", name)
        del img, label_mask, pred
        
    
    def nearest_sample(self, Rnd_G, img, lbl, unlb_path, unlist):
        device=self.device
        ...
        for i in range(0,len(unlb_samples)):
            print("Unlabeled sample = ",unlist[i])
            unlb_img = self.read_img(unlb_path+unlb_samples[i])#.to(device)
            unlb_lbl = lbl.cpu().numpy()#.to(device)
            #thresh_unlb = (int(num_classes) - 1) + 0.5
            #unlb_lbl[unlb_lbl < 0.5] = 0.0  # maybe some voxels is a minus value
            #unlb_lbl[unlb_lbl > thresh_unlb] = 255
            sample_unlb_unedit = {'image': unlb_img, 'label': unlb_lbl, 'case_name':unlist[i][:-7]}

            while True:
                sample_unlb_crop = Rnd_G(sample_unlb_unedit)

                unlb_labels = torch.unique(sample_unlb_crop['label'])
                print(unlb_labels)

                if len(unlb_labels)>0:
                    break

            #learned_proto = get_protoype()

            img_unlb = sample_unlb_crop['image']
            img_unlb = img_unlb.to(device)
            
            img_unlb = img_unlb.unsqueeze(0)
            img_unlb = img_unlb.unsqueeze(0)
            #print("Turning input, label ",img.shape, lbl.shape)
            fout_unlb, out_unlb = self.model(img_unlb) #.unsqueeze(0)) #, use_classifier=False)
            #print("Model out, embed, enc", out.shape, fout.shape)

            #print("Encoder output, img", out.shape, img.shape[-3:])
            out_unlb = out_unlb.squeeze(0)
            
            out_unlb=out_unlb.detach().cpu().numpy()
            print("Labeled sample out, Unlabeled out", out.shape, out_unlb.shape)
            
            squared_dist = np.sum((out-out_unlb)**2, axis=0).mean()
            dist = np.sqrt(squared_dist)
            
            
            print("Distance = ",dist)
            step_dist[unlist[i]]=dist
            del unlb_img, img_unlb,out_unlb,fout_unlb,squared_dist,dist,sample_unlb_unedit,sample_unlb_crop
        del out, fout, img, lbl
        return step_dist
    ...
    def train(self, optimizer, dice_weight, ce_weight, cur_epoch, train_loader, metrics=None, print_int=10, n_iter=1, snapshot_path = None):
        ...
        for i_batch, sampled_batch in enumerate(train_loader):
            ...
            ########################################################################################################
            rest_loss_values=rest_loss_values=(1.0-self.kd_loss)/2.0
