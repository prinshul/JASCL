...
def save_ckpt(t, path, model, epoch):
    """ save current model
    """
    state = {
        "epoch": epoch,
    ...
    # save mask as pickle file
    mask = model.save_masks()
    fname='mask'+str(t.step)+'.pkl'
...
def main(opts):
    ...
    optimizer = torch.optim.AdamW(model.model.parameters(), lr=1e-2, eps=1e-8, betas=(0.9, 0.999), weight_decay=1e-5)
    ...
    while cur_epoch < max_epoch and not opts.test:
        ...
        if mean_val_dice > highest_val_dice:
            ...
            epo_val = cur_epoch + 1
            save_ckpt(task, checkpoint_path, model, cur_epoch)
