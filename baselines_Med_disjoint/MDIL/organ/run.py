...
def main(opts):
    ...
    if not opts.test:
        ...
        while cur_epoch < max_epoch and not opts.test:
            # =====  Train  =====
            start = time.time()
            class_dice_train, train_dice = model.train(optimizer, lr_scheduler, dice_weight, ce_weight, cur_epoch=cur_epoch, train_loader=train_loader,
            ...
            if cur_epoch % 50 == 0:
                ...
                with open('cil/MDIL/organ/results/dice_step_{}_{}_test_{}_dynamic.txt'.format(task.step, cur_epoch, opts.network_arch), 'w') as file:
        ...
        with open('cil/MDIL/organ/results/dice_step_{}_train_{}_dynamic.txt'.format(task.step, opts.network_arch), 'w') as file:
            for key, value in class_wise_dice_train_dict.items():
                file.write(f'{key}: {value}\n')
            
        with open('cil/MDIL/organ/results/dice_step_{}_val_{}_dynamic.txt'.format(task.step, opts.network_arch), 'w') as file:
    ...
    if opts.test or test_end:
        ...
        if opts.test and opts.ckpt is not None:
            ...
            # checkpoint_path = f"checkpoints/step/{task_name}/FT_0_erfnet_RA_parallel_dynamic_old_save_new.pth"
        ...
        with open('cil/MDIL/organ/results/dice_step_{}_test_{}_dynamic.txt'.format(task.step, opts.network_arch), 'w') as file:
