...
from methods.segmentation_module import get_old_model
...
def main(opts):
    ...
    if opts.step == 0:  # if step 0, we don't need to instance the model_old
        ...
        model_old = get_old_model(opts, num_classes=len(task.old_classes))
    ...
    if task.step > 0:
        ...
        if os.path.exists(path):
            step_checkpoint = torch.load(path, map_location="cpu")
            model_old.load_state_dict(step_checkpoint['model_state']['model'], strict=True)  # Load also here old parameters
    ...
    # lr_scheduler = LinearWarmupCosineAnnealingLR(optimizer, warmup_epochs=1, max_epochs=200)
    ...
    while cur_epoch < max_epoch and not opts.test:
        # =====  Train  =====
        start = time.time()
        class_dice_train, train_dice = model.train(model, optimizer, dice_weight, ce_weight, cur_epoch=cur_epoch, train_loader=train_loader,
        ...
        if cur_epoch % 20 == 0:
            ...
            with open('cil/JHH_new/organ/results/dice_step_{}_{}_test_{}_dynamic.txt'.format(task.step, cur_epoch, opts.network_arch), 'w') as file:
    ...
    with open('cil/JHH_new/organ/results/dice_step_{}_train_{}_dynamic.txt'.format(task.step, opts.network_arch), 'w') as file:
        for key, value in class_wise_dice_train_dict.items():
            file.write(f'{key}: {value}\n')
        
    with open('cil/JHH_new/organ/results/dice_step_{}_val_{}_dynamic.txt'.format(task.step, opts.network_arch), 'w') as file:
    ...
    if opts.test_old:
            test_metrics = StreamSegMetrics(len(task.get_old_labels(bkg=True)), task.get_n_classes()[0])
    else:
            test_metrics = StreamSegMetrics(len(task.get_order()), task.get_n_classes()[0])

    class_dice_test, test_dice = model.test(dice_weight, ce_weight, loader=test_loader_all, metrics=test_metrics, ret_samples_ids=None, cur_epoch=cur_epoch, snapshot_path = snapshot_path, test_old = opts.test_old)
    ...
    with open('cil/JHH_new/organ/results/dice_step_{}_test_{}_dynamic.txt'.format(task.step, opts.network_arch), 'w') as file:
