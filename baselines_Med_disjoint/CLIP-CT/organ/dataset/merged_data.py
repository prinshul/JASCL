...
class RandomGenerator(object):
    def __init__(self, step, output_size, mode):
    ...
    def __call__(self, sample):
        ...
        # if self.step > 0:
        #     label_unq = torch.unique(label)
        #     for class_num in range(self.num_classes):
        #         prediction = self.model(image.cuda())
...
class MergedSegmentation(data.Dataset):
    ...
    def __init__(self, root, base_dir, split, num_classes, transform=None):
        ...
        # for i in range(1, 16):
        #     print("Class {}".format(i), set(self.class_to_images_[i]).intersection(self.class_to_images_[18]))    
