...
class FSSDataset(data.Dataset):
    def __init__(self,
        ...
        if not self.multi_idxs:  # step 0 or test
                self.dataset = Subset(self.full_data, self.cur_label, idxs, mask_label = train, transform_error = transform_error, transform=transform, target_transform=None)
        else:
            dts_list = []
            count = 0
            if 0 in idxs:
            ...
            for i in range(1, len(self.labels)+1):
                dts_list.append(Subset(self.full_data, self.cur_label, idxs[i], mask_label = train,
                                       transform_error = transform_error,transform=transform, target_transform=target_transform[i]))
