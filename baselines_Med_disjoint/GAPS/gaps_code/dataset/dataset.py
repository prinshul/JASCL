...
class FSSDataset(data.Dataset):
    def __init__(self,
        ...
        self.full_data = self.make_dataset(root, base_dir, train, num_classes)
        print("\n\n",train,self.full_data)
        ...
        #self.cur_label = self.labels # strict IL
        self.cur_label = self.labels_old + self.labels   # non-strict IL
        #print("Non Strict IL CLasses: ", self.cur_label)
        print("Strict IL CLasses: ", self.cur_label)
        ...
        self.inverted_order = {self.order.index(lb):lb for lb in self.order}
        if train:
        ...
        if train == 'test':
            ...
            if task.disjoint:
                for cl, img_set in self.full_data.class_to_images.items():
                    if cl in self.labels and (cl != 0):
                        idxs = idxs.union(img_set)
            idxs = list(idxs)
            ...
            if task.input_mix == 'both':
                ...
                for cl in self.labels_old:
                    ...
                    idxs[0].extend(self.full_data.class_to_images[cl][ishot*3: ishot*3+nshot])
                target_transform[0] = self.get_mapping_transform(self.labels_old, masking=True,
                                                                 masking_value=masking_value)
                #print("ch 1",idxs,target_transform)
            
            if train=='exemplar' or train=='exemplar1' or train=='exemplar2':
                for i, cl in enumerate(self.labels_old):
                    images_of_class = self.full_data.class_to_images[cl]
                    #print("images_of_class ",images_of_class)
                    if FILTER_FSL:
                        # filter images containing unseen or actual classes
                        for cl_, img_set in self.full_data.class_to_images.items():
                            if cl_ != cl and cl_ not in self.labels and (cl_ != 0):
                                images_of_class = images_of_class.difference(img_set)
                    idxs[i+1] = images_of_class[ishot*1: ishot*1+nshot]
            ...
            #print("idxs ",idxs,lbls,target_transform)
        
        # make the subset of the dataset
        self.indices = []
        if not self.multi_idxs:  # step 0 or test
                self.dataset = Subset(self.full_data, self.cur_label, idxs, mask_label = train, transform=transform, target_transform=None)
        else:
            dts_list = []
            count = 0
            #print("idxs ",idxs)
            if 0 in idxs:
                dts_list.append(Subset(self.full_data, self.cur_label, idxs[0], mask_label = train,
            ...
            if train=='exemplar' or train=='exemplar1' or train=='exemplar2':
                for i in range(1, len(self.labels_old)+1):
                    dts_list.append(Subset(self.full_data, self.labels_old, idxs[i], mask_label = train,
                                           transform=transform, target_transform=target_transform[i]))
                    self.class_to_images[self.labels_old[i-1]] = list(range(count, count+len(idxs[i])))
                    count += len(idxs[i])
            else:
                for i in range(1, len(self.labels)+1):
                    dts_list.append(Subset(self.full_data, self.cur_label, idxs[i], mask_label = train,
                                           transform=transform, target_transform=target_transform[i]))
