...
class MergedSegmentation(data.Dataset):
    ...
    def __init__(self, root, base_dir, split, num_classes, transform=None):
        ...
        if self.split=="exemplar1":
            splits_dir="/hdd2/cil/running_base/GAPS/gaps_code/data/merged/split/"
            self.data_dir =  "/hdd2/cil/running_base/GAPS/gaps_code/data/merged/"
        if self.split=="exemplar2":
            splits_dir="/hdd2/cil/running_base/GAPS/gaps_code/data/merged/split/"
            self.data_dir =  "/hdd2/cil/running_base/GAPS/gaps_code/data/merged/"
        ...
        if self.split == 'train':
            ...
            self.class_to_images_ = pkl.load(open(splits_dir + path, 'rb'))
        elif self.split == 'exemplar1':
            print("taking exemplar 1 data")
            path = '/inverse_dict_new_exemplar1.pkl'
            self.class_to_images_ = pkl.load(open(splits_dir + path, 'rb'))
        elif self.split == 'exemplar2':
            print("taking exemplar 2 data")
            path = '/inverse_dict_new_exemplar2.pkl'
            self.class_to_images_ = pkl.load(open(splits_dir + path, 'rb'))
        else:
    ...
    def __getitem__(self, idx):
        if self.split == "train":
            ...
            slice_name = self.sample_list[idx].strip('\n')
            img_path = os.path.join(self.data_dir, 'dataset', 'images', 'exemplar', slice_name + '.nii.gz')
            image = sitk.ReadImage(img_path)
            label_path = os.path.join(self.data_dir, 'dataset', 'annotations', 'exemplar', slice_name + '.nii.gz')
            label = sitk.ReadImage(label_path)
            origin = np.array(image.GetOrigin())
            spacing = np.array(image.GetSpacing())
            image = sitk.GetArrayFromImage(image)
            ...
            slice_name = self.sample_list[idx].strip('\n')
            img_path = os.path.join(self.data_dir, 'dataset', 'images', 'exemplar1', slice_name + '.nii.gz')
            image = sitk.ReadImage(img_path)
            label_path = os.path.join(self.data_dir, 'dataset', 'annotations', 'exemplar1', slice_name + '.nii.gz')
            label = sitk.ReadImage(label_path)
            origin = np.array(image.GetOrigin())
            spacing = np.array(image.GetSpacing())
            image = sitk.GetArrayFromImage(image)
            ...
            slice_name = self.sample_list[idx].strip('\n')
            img_path = os.path.join(self.data_dir, 'dataset', 'images', 'exemplar2', slice_name + '.nii.gz')
            image = sitk.ReadImage(img_path)
            label_path = os.path.join(self.data_dir, 'dataset', 'annotations', 'exemplar2', slice_name + '.nii.gz')
            label = sitk.ReadImage(label_path)
            origin = np.array(image.GetOrigin())
            spacing = np.array(image.GetSpacing())
            image = sitk.GetArrayFromImage(image)
