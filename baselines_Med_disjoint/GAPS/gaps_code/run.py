...
def step_more_with_replay(opts,task,logger,rank,device):
    ...
    train_iterations = 1 if task.step == 0 else 20 // task.nshot
    if opts.iter is not None:
        opts.epochs = opts.iter // (len(exemp_loader) * train_iterations)
    opts.max_iter = opts.epochs * len(exemp_loader) * train_iterations
    if opts.max_iter == 0:
        opts.max_iter = 1
    ...
    logger.info(f"Total batch size is {min(opts.batch_size, len(train_dst))}")
    logger.info(f"Train loader contains {len(train_loader)} iterations per epoch, multiplied by {train_iterations}")
    logger.info(f"Exemp loader contains {len(exemp_loader)} iterations per epoch")
    logger.info(f"Total iterations are {opts.max_iter}, corresponding to {opts.epochs} epochs")
    ...
    if task.step > 0:
        ...
        #print(step_ckpt['model_state']['model']['outc.bias'].shape)base_ckpt=step_ckpt
    ...
    max_epoch = opts.max_epochs
    max_iterations = opts.max_epochs * len(exemp_loader)          

    logger.info("{} iterations per epoch. {} max iterations ".format(len(exemp_loader), max_iterations))
    writer = SummaryWriter(snapshot_path + '/log')
    # load progress bar
    iterator = tqdm(range(max_epoch), ncols=70)
    optimizer = torch.optim.AdamW(model.model.parameters(), lr=5e-4, eps=1e-8, betas=(0.9, 0.999), weight_decay=1e-5)
    ...
    while cur_epoch <= max_epoch and not opts.test:
        # =====  Train  =====
        start = time.time()
        class_dice_train, train_dice = model.train(optimizer, dice_weight, ce_weight, cur_epoch=cur_epoch,
                train_loader=exemp_loader, metrics=train_metrics, print_int=opts.print_interval,
                                 n_iter=train_iterations, snapshot_path = snapshot_path)
        # train_score = train_metrics.get_results()
        end = time.time()
