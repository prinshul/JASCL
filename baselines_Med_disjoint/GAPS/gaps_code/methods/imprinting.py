...
class WeightImprinting(Trainer):
    ...
    def __init__(self, task, device, logger, opts, config_vit):
        super().__init__(task, device, logger, opts, config_vit)
        self.masking = True
        self.normalize_weight = False
        self.compute_score = False

    def warm_up_(self, loader, old_model, epochs=5):
        ...
        # sum_features = torch.zeros(classes[-1], 432).to(self.device)
        ...
        #         output, embedding = model(image_batch)
        #         cl = label_batch.unique().cpu().numpy()  # get list of classes
        #         for c in new_classes:
        #             if c in cl:
        #                 feat = embedding[:,c,:]  # F x P (pixels of class c)
        #                 feat =  F.normalize(feat, dim=0).sum(dim=0)
        #                 feat = feat.reshape(-1)
        ...
        # features = F.normalize(sum_features, dim=0)
        # weight = torch.cat((old_model['segmentation_head.weight'].cuda(), features.view(5, 16, 3, 3, 3)))
        
        # not_equal = not torch.allclose(model.segmentation_head.weight.data , weight, atol=1e-5)
        # assert not_equal, "Tensors are equal"

        # model.segmentation_head.weight.data = weight
        model = self.model.module if self.distributed else self.model
        model.eval()
        classes = self.task.get_n_classes()
        old_classes = 0
        for c in classes[:-1]:
            old_classes += c
        sum_features = torch.zeros(self.num_classes, self.model.outc.in_channels).to(self.device)
        oi_acc = torch.zeros(self.num_classes, self.num_classes - 1).to(self.device)
        count = torch.zeros(self.num_classes).to(self.device)

        for ep in range(epochs):
            with torch.no_grad():
                for idx, sampled_batch in enumerate(loader):
                    inputs, labels = sampled_batch['image'], sampled_batch['label']
                    inputs = inputs.to(self.device, dtype=torch.float32)
                    labels = labels.to(self.device, dtype=torch.long)

                    outputs = model(inputs.unsqueeze(0))  # Batch size of 1
                    cl = labels.unique().cpu().numpy()

                    for c in range(self.num_classes):
                        if c in cl:
                            feat = outputs[:, labels == c]  # Channels x Voxels

                            if self.compute_score:
                                oi = outputs[:, labels != c]  # Channels x Voxels
                                oi_acc[c] += oi.mean(dim=1)

                            feat = F.normalize(F.normalize(feat, dim=0).sum(dim=1), dim=0)
                            sum_features[c] += feat
                            count[c] += 1

        assert torch.any(count != 0), "Error, a novel class has no voxels!"
        ...
        if self.normalize_weight:
            if self.compute_score:
                ...
                for c in range(self.num_classes):
                    f = (self.model.outc.weight[0, c] * scores[c])
                    features[c] += f.sum(dim=0)

        # Imprint the weights into the model
        self.model.outc.weight.data[:, :, 0, 0, 0] = features.to(self.model.outc.weight.device)
