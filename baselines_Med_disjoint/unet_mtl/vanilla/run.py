...
def main(opts):
    ...
    print("test:single heads for each step.\n")
    
    print("\nNum classes:", opts.num_classes)
    print("Total epochs : ",max_epoch)
    print("Train_epochs : ",train_epoch)
    print("Batch size : ",batch_size)
    
    
    task_classes = [16,5,6,4]
    curr_classes = 0
    for xi in range(opts.step+1):
        curr_classes+=task_classes[xi]
    
    # train/val here
    while cur_epoch <= max_epoch and not opts.test:
        # =====  Train  =====
        
        if cur_epoch<train_epoch:
            start = time.time()
            ...
            highest_train_dice, epo_train, 0, 0))
        # =====  Validation  =====
        ...
        if cur_epoch>=train_epoch:
            ...
            class_dice_val, val_dice = model.mtl_validate(optimizer, dice_weight, ce_weight, cur_epoch=cur_epoch, train_loader=val_loader,
                                     metrics=val_metrics, print_int=opts.print_interval,
                                     n_iter=train_iterations, snapshot_path = snapshot_path)
        ...
        if cur_epoch % 50 == 0:
            print("\n\n*** Test the model on previous classes...")
