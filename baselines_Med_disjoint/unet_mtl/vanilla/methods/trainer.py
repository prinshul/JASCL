...
class Trainer:
    ...
    def train(self, optimizer, dice_weight, ce_weight, cur_epoch, train_loader, metrics=None, print_int=10, n_iter=1, snapshot_path = None):
        ...
        for i_batch, sampled_batch in enumerate(train_loader):
            ...
            outputs = model(image_batch,'train')
            #print(image_batch.shape, outputs.shape)
            # torch.Size([1, 1, 128, 160, 96]) torch.Size([1, 21, 128, 160, 96])
    ...
    def mtl_validate(self, optimizer, dice_weight, ce_weight, cur_epoch, train_loader, metrics=None, print_int=10, n_iter=1, snapshot_path = None):
        """Train and return epoch loss"""
        if metrics is not None:
            metrics.reset()
        logger = self.logger
        ...
        for i_batch, sampled_batch in enumerate(train_loader):
            ...
            image_batch, label_batch = image_batch.cuda(), label_batch.cuda()
            # outputs, feat = model(image_batch)
            outputs = model(image_batch,'test')
            #print(image_batch.shape, outputs.shape)
            # torch.Size([1, 1, 128, 160, 96]) torch.Size([1, 21, 128, 160, 96])
            # print("Prediction", torch.unique(outputs))
            loss_ce_1 = ce_loss(outputs, label_batch[:].long())
    ...
    def freeze_model(self):
        for name, param in self.model.named_parameters():
            if name.startswith('outc') or name.startswith('up') or name.startswith('down') or name.startswith('inc') or name.startswith('single_head'):
