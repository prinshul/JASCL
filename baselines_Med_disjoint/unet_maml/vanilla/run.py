...
def main(opts):
    ...
    test_loader_all = DataLoader(test_dst_all, batch_size=batch_size, shuffle = False,
                             ...
                             worker_init_fn=worker_init_fn)
    '''
    train_loader = BatchMetaDataLoader(train_dst,
                                     batch_size=batch_size,
                                     shuffle=True,
                                     num_workers=opts.num_workers,
                                     pin_memory=True)
                                     
    print("Batch train metaloader",len(train_loader))
    
    
    val_loader = BatchMetaDataLoader(val_dst,
                                    batch_size=batch_size,
                                     shuffle=True,
                                     num_workers=opts.num_workers,
                                     pin_memory=True)
    
    print("Batch val metaloader",len(val_dataloader))
    '''
    print("train_loader")
    ...
    print("\n\nMeta learning with reg\n")
    print("--step_size ",opts.step_size)
    print("--s_ratio ",opts.s_ratio)
    print("--s_norm ",opts.s_norm)
    print("lbda1 with 1. and lbda2 with 1. increased up the loss to 30+")
    print("--lbda1 = ",opts.lbda1)
    print("--lbda2 = ",opts.lbda2)
    
    print("\nNum classes:", opts.num_classes)
    ...
    while cur_epoch <= max_epoch and not opts.test:
        # =====  Train  =====
        start = time.time()
        class_dice_train, train_dice = model.maml_train(opts, optimizer, dice_weight, ce_weight, cur_epoch=cur_epoch, train_loader=train_loader,
