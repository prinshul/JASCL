...
class MergedSegmentation(data.Dataset):
    ...
    def __init__(self, root, base_dir, split, num_classes, transform=None):
        ...
        self.exemp_path = "/hdd2/cil/running_base/data_replay_Diffusion/data_replay/data/merged"
        ...
        if self.split == 'exemplar':
            ...
            print("For exemplars ",os.path.join(self.exemp_path+"/split", self.split + '.txt'))
    ...
    def __getitem__(self, idx):
        if self.split == "train":
            ...
            slice_name = self.sample_list[idx].strip('\n')
            img_path = os.path.join(self.exemp_path, 'dataset', 'images', 'exemplar', slice_name + '.nii.gz')
            #print(img_path)
            image = sitk.ReadImage(img_path)
            label_path = os.path.join(self.exemp_path, 'dataset', 'annotations', 'exemplar', slice_name + '.nii.gz')
            label = sitk.ReadImage(label_path)
            origin = np.array(image.GetOrigin())
            spacing = np.array(image.GetSpacing())
            image = sitk.GetArrayFromImage(image)
