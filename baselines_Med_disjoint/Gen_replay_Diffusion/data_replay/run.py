...
def step_more_with_replay(opts,task,logger,rank,device):
    ...
    for i_batch, sampled_batch in enumerate(train_loader):
            ...
            print(np.unique(image_batch),np.unique(label_batch))
    ...
    print("The length of concat set is: {}".format(len(concat_dst)))
    ...
    for ci_batch, csampled_batch in enumerate(concat_loader):
            print("Patient ids", csampled_batch['case_name'])
            print(csampled_batch['image'].shape, csampled_batch['label'].shape)
    ...
    train_iterations = 1 if task.step == 0 else 20 // task.nshot
    if opts.iter is not None:
        opts.epochs = opts.iter // (len(concat_loader) * train_iterations)
    opts.max_iter = opts.epochs * len(concat_loader) * train_iterations
    if opts.max_iter == 0:
        opts.max_iter = 1
        
    # logger.info(f"Total batch size is {min(opts.batch_size, len(train_dst)) * world_size}")
    logger.info(f"Total batch size is {min(opts.batch_size, len(concat_dst))}")
    logger.info(f"Train loader contains {len(train_loader)} iterations per epoch, multiplied by {train_iterations}")
    logger.info(f"Concat loader contains {len(concat_loader)} iterations per epoch")
    logger.info(f"Total iterations are {opts.max_iter}, corresponding to {opts.epochs} epochs")
