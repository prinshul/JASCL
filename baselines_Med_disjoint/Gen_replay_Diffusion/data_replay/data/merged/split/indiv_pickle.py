...
def create_inverse_dict_train():
    ...
    with open("exemplar.txt", "r") as f:
        patient_numbers = [int(line.strip()) for line in f]

    # Step 2: Traverse only the /val directory
    folder = "../dataset/annotations/exemplar"
    for filename in os.listdir(folder):
        print(f"{folder}/{filename}") 
        # Extracting patient number from filenames like '0.nii.gz'
        patient_number = int(filename.split('.')[0])
    ...
    with open("inverse_dict_exemplar.pkl", "wb") as f:
        pickle.dump(inverse_dict, f)
    with open("inverse_dict_new_exemplar.pkl", "wb") as f:
        pickle.dump(inverse_dict, f)
