...
def main(opts):
    ...
    print("\n\nSelf supervised learning with unsupervised - hard negative mining\n")
    print("tau_plus : ", opts.tau_plus)
    print("beta : ", opts.beta)
    print("temperature : ", opts.temperature)
    print("Rotation weight : ", opts.rot_weight)
    print("Num classes : ", opts.num_classes)
    print("ssl epochs : ",opts.ssl_steps)
    print("Total epochs : ",max_epoch)
    # train/val here
    while cur_epoch <= max_epoch and not opts.test:
        # =====  Train  =====
        
        if cur_epoch<opts.ssl_steps:
            start = time.time()
            mean_con_loss = model.ssl_train(opts, optimizer, dice_weight, ce_weight, cur_epoch=cur_epoch, train_loader=train_loader,
                                     metrics=train_metrics, print_int=opts.print_interval,
                                     n_iter=train_iterations, snapshot_path = snapshot_path)
            end = time.time()
        else:
            start = time.time()
            class_dice_train, train_dice = model.train(opts, optimizer, dice_weight, ce_weight, cur_epoch=cur_epoch, train_loader=train_loader,
                                     metrics=train_metrics, print_int=opts.print_interval,
                                     n_iter=train_iterations, snapshot_path = snapshot_path)
