...
class Trainer:
    ...
    def hard_neg_loss(self,out_1,out_2,tau_plus,batch_size,beta,temperature):
        # neg score
    ...
    def ssl_train(self, opts, optimizer, dice_weight, ce_weight, cur_epoch, train_loader, metrics=None, print_int=10, n_iter=1, snapshot_path = None):
        ...
        for i_batch, sampled_batch in enumerate(train_loader):
            ...
            aug1_image0, aug2_image0 = aug1_image0.unsqueeze(0).cuda(), aug2_image0.unsqueeze(0).cuda()
            aug1_image1, aug2_image1 = aug1_image1.unsqueeze(0).cuda(), aug2_image1.unsqueeze(0).cuda()
            
            #print(aug1_image0.shape)
            ## out,ssl,rot
            #img0_aug = torch.cat([aug1_image0,aug2_image0],dim=0)
            #img0_aug = torch.cat([aug1_image0,aug2_image0],dim=0)
            
            _,ssl_aug1_img0,_ = model(aug1_image0)
            _,ssl_aug2_img0,_ = model(aug2_image0)
            
            _,ssl_aug1_img1,_ = model(aug1_image1)
            _,ssl_aug2_img1,_ = model(aug2_image1)
            # print(image_batch.shape, outputs.shape)
            # print("Prediction", torch.unique(outputs))
            #print("i_batch, ssl = ",i_batch, ssl_aug1_img0.shape)
            
            '''
            bsz = 1
            f1, f2 = torch.split(ssl, [bsz, bsz], dim=0)
            print('f1 f2 ', f1.shape,f2.shape)
            features = torch.cat([f1.unsqueeze(1), f2.unsqueeze(1)], dim=1)
            print("features ",features.shape)
            ssl =  torch.Size([2, 512])
            f1 f2  torch.Size([1, 512]) torch.Size([1, 512])
            features  torch.Size([1, 2, 512])
            
            features = torch.cat([ssl_aug1_img0.unsqueeze(1), ssl_aug2_img0.unsqueeze(1),
                                  ssl_aug1_img1.unsqueeze(1), ssl_aug2_img1.unsqueeze(1)], dim=0)
            #print("features ",features.shape)
            '''
            out1 = torch.cat([ssl_aug1_img0,ssl_aug1_img1], dim=0)
            out2 = torch.cat([ssl_aug2_img0,ssl_aug2_img1], dim=0)
            #print(out1.shape, out2.shape)
            con_loss = self.hard_neg_loss(out1,out2,tau_plus,batch_size,beta,temperature)
            #con_loss = criterion(features,mask=mask)
    ...
    def train(self, opts, optimizer, dice_weight, ce_weight, cur_epoch, train_loader, metrics=None, print_int=10, n_iter=1, snapshot_path = None):
        ...
        for i_batch, sampled_batch in enumerate(train_loader):
            ...
            for ang in range(4):
                ...
                '''
                rloss = rot_loss(rot,curr_lbl)
                print(rloss)
                rloss = rot_weight*rloss
                '''
