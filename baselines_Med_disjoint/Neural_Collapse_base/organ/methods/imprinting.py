...
class WeightImprinting(Trainer):
    ...
    def __init__(self, task, device, logger, opts, config_vit, class_wise_embeddings_previous):
        super(WeightImprinting, self).__init__(task, device, logger, opts, config_vit, class_wise_embeddings_previous)
        ...
        self.keys = nn.Parameter(
            torch.FloatTensor(self.dim, self.dim).normal_(0., math.sqrt(2 / 4)))
    ...
    def imprint_weights_class(self, model, weight, cl):
        ...
        model.outc.weight[cl,:].data = weight[cl, :]
        return model
    ...
    def warm_up_(self, dataset, old_model, epochs=5):
        ...
        for c in new_classes:
            ...
            while weight is None and count < 10:
                ...
                print(model.outc.weight.shape)
                if wc is not None:
                    weight = self.compute_weight(wc, model.outc.weight)
                count += 1
            # print(model.outc.weight.shape, weight.shape)
            if weight is not None:
                model = self.imprint_weights_class(model, weight, c)
            else:
    ...
    def warm_up_(self, dataset, epochs=1):
        ...
        for c in new_classes:
            ...
            while weight is None and count < 10:
                ds = dataset.get_k_image_of_class(cl=c, k=self.task.nshot)  # get K images of class c
                wc = get_prototype(model, ds, c, self.device, interpolate_label=False, return_all=True)
                print(model.cls.cls[0].weight.shape)
                if wc is not None:
