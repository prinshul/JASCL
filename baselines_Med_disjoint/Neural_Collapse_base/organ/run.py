...
def main(opts):
    ...
    # checkpoint_path_etf = f"checkpoints/step/{task_name}/{name}_{opts.step}_{opts.network_arch}_dynamic_ETF.pth"
    ...
    if opts.step == 0:
        # class_mean_calculator = torch.tensor([0] * 128).cuda()
        class_mean_calculator = None
    else:
        previous_step = opts.step - 1
        class_mean_calculator = torch.load('cil/Neural_Collapse/organ/class_mean/class_mean_{}.pth'.format(previous_step))
    model = get_method(opts, task, device, logger, config_vit, class_mean_calculator)
    ...
    if task.step > 0:
        ...
        # old_model_etf = step_ckpt_etf['model_state']
        
        if opts.load_weight_strategy:
            model.load_dict_imprint(old_model, class_mean_calculator, strict=False)  # False because of incr. classifiers
        else:
    ...
    # else:
    etf_vec = torch.load('cil/Neural_Collapse/organ/ortho_vec/etf_vec.pth')
    etf_vec = etf_vec.T
    tmp = etf_vec[:opts.num_classes].data.clone()
    tmp = tmp.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)
    model.model.outc.weight.data = tmp
    for param in model.model.outc.parameters():
        param.requires_grad = False
    model.model.cuda()
    for name, param in model.model.named_parameters():
        print(name, param.requires_grad)
    # print("---------------------------")
    # for name, param in model.model_etf.named_parameters():
    #     print(name, param.requires_grad)
...
# xxx Model warm up
    ...
    while cur_epoch < max_epoch and not opts.test:
        # =====  Train  =====
        start = time.time()
        class_dice_train, train_dice, class_mean_calculator, model_etf = model.train(optimizer, dice_weight, ce_weight, cur_epoch=cur_epoch, train_loader=train_loader,
        ...
        if cur_epoch % 50 == 0:
            ...
            with open('cil/Neural_Collapse/organ/results/dice_step_{}_{}_test_{}_dynamic.txt'.format(task.step, cur_epoch, opts.network_arch), 'w') as file:
        ...
        if mean_val_dice > highest_val_dice:
            ...
            # save_ckpt(checkpoint_path_etf, model_etf, cur_epoch)
    ...
    torch.save(class_mean_calculator, 'cil/Neural_Collapse/organ/class_mean/class_mean_{}.pth'.format(task.step))
    ...
    with open('cil/Neural_Collapse/organ/results/dice_step_{}_train_{}_dynamic.txt'.format(task.step, opts.network_arch), 'w') as file:
        for key, value in class_wise_dice_train_dict.items():
            file.write(f'{key}: {value}\n')
        
    with open('cil/Neural_Collapse/organ/results/dice_step_{}_val_{}_dynamic.txt'.format(task.step, opts.network_arch), 'w') as file:
    ...
    with open('cil/Neural_Collapse/organ/results/dice_step_{}_test_{}_dynamic.txt'.format(task.step, opts.network_arch), 'w') as file:
