...
class CosineClassifier(nn.Module):
    def __init__(self, classes, channels=256):
        super().__init__()
        self.channels = channels
        self.cls = nn.Conv3d(channels, 27, 1, bias=False)
        self.scaler = 10.
        self.classes = classes
        self.tot_classes = 0
        for lcl in classes:
    ...
    def forward(self, x):
        x = F.normalize(x, p=2, dim=1)
        out = self.softabs(F.conv3d(torch.tanh(x), torch.tanh(F.normalize(self.cls.weight, dim=1, p=2))))
        return out
    ...
    def softabs(self, x, steepness=10):
        act=torch.sigmoid(steepness * (x - 0.5)) + torch.sigmoid(steepness * (-x - 0.5))
