...
class UNet(nn.Module):
    def __init__(self, in_ch, base_ch, scale=[[1,2,2], [2,2,2], [2,2,2], [2,2,2]], kernel_size=[[1,3,3], [2,3,3], [3,3,3], [3,3,3], [3,3,3]], num_classes=1, block='ConvNormAct', pool=True, norm='bn'):
        ...
        self.ssl_head = nn.Linear(160*32*10*6,512)
        
        self.rot_head = nn.Sequential( 
                            nn.Linear(160*32*10*6,256),
                            nn.ReLU(),
                            nn.Linear(256,4) 
                        )
    ...
    def forward(self, x): 
        ...
        fts = x5.clone()
        encoded = fts.view(fts.size(0), -1)
        #print(encoded.shape) torch.Size([2, 307200])
        ssl = F.normalize(self.ssl_head(encoded),dim=1)
        rot = self.rot_head(encoded)
