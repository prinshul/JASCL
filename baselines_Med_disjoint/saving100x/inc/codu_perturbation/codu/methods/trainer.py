...
class Trainer:
    def __init__(self, task, device, logger, opts, config_vit):
        ...
        # MODIFIED: Include prototype replay requirement
        self.need_model_old = (self.step > 0 or  # ADD THIS LINE
                            self.opts.born_again or self.opts.mib_kd > 0 or self.opts.loss_kd > 0 or
        ...
        if self.need_model_old:
            ...
            logger.info("=" * 80)
            logger.info("OLD MODEL CREATED")
            logger.info(f"Step: {self.step}")
            logger.info(f"Model old type: {'new structure' if model_old_as_new else 'old structure'}")
            logger.info("=" * 80)
        
        # Add PKD and Contrastive losses for incremental learning
        if self.step > 0 and self.model_old is not None:
            ...
            logger.info("=" * 80)
            logger.info("PROTOTYPE REPLAY INITIALIZATION")
            logger.info("=" * 80)
            logger.info(f"Step: {self.step}")
            logger.info(f"Old classes: {len(self.labels_old)}")
            logger.info(f"New classes: {len(self.labels)}")
            logger.info(f"PKD weight: {self.pkd_weight}")
            logger.info(f"Contrastive weight: {self.cont_weight}")
            logger.info("=" * 80)
        else:
            ...
            if self.step > 0:
                logger.warning("!" * 80)
                logger.warning("WARNING: Step > 0 but model_old is None!")
                logger.warning("Prototype replay will not work properly!")
                logger.warning("!" * 80)
    ...
    def train(self, optimizer, dice_weight, ce_weight, cur_epoch, train_loader, metrics=None, print_int=10, n_iter=1, snapshot_path=None):
        ...
        for i_batch, sampled_batch in enumerate(train_loader):
            ...
            if self.opts.step == 0:
                ...
                with torch.no_grad():
                    logits_old, features_old, _ = self.model_old(image_batch, ret_intermediate=True)
                ...
                # Around line 520 - Forward pass with fake features
                logits, feat, logits_fake = model(image_batch, fake_features=fake_features, ret_intermediate=True)

                if i_batch == 0:
                    logger.info(f"Feature map shape: {feat.shape}")
                    if logits_fake is not None:
                        logger.info(f"Fake logits shape (raw): {logits_fake.shape}")
                    else:
                        logger.warning("Fake logits is None!")
                ...
                # 2. Prototype replay loss
                # 2. Prototype replay loss
                loss_proto_replay = torch.tensor(0.0).to(device)
                if logits_fake is not None:
                    # logits_fake shape: [21, 1, 90, 1]
                    # First dimension (21) is num_classes
                    # Third dimension (90) is N_fake_samples
                    
                    if i_batch == 0:
                        logger.info(f"Fake logits original shape: {logits_fake.shape}")
                    
                    # Reshape: [num_classes, 1, N_samples, 1] -> [N_samples, num_classes]
                    # Remove the singleton dimensions (dim 1 and 3), then transpose
                    logits_fake_reshaped = logits_fake.squeeze(1).squeeze(-1).transpose(0, 1)  # [90, 21]
                    
                    # Create fake labels: all should be background (class 0)
                    n_fake_samples = logits_fake_reshaped.shape[0]
                    fake_labels = torch.zeros(n_fake_samples, dtype=torch.long, device=device)
                    
                    if i_batch == 0:
                        logger.info(f"Fake logits reshaped: {logits_fake_reshaped.shape}")
                        logger.info(f"Fake labels shape: {fake_labels.shape}")
                    
                    loss_proto_replay = ce_loss(logits_fake_reshaped, fake_labels)
                    
                    if i_batch % 10 == 0:
                        logger.info(f"  Loss Prototype Replay: {loss_proto_replay.item():.4f}")
                ...
                if self.pkd_criterion is not None:
                    ...
                    # _, features_new, _ = model(image_batch, ret_intermediate=True) 
                    features_new = feat 
    ...
    def validate(self, dice_weight, ce_weight, loader, metrics, ret_samples_ids=None, novel=False, cur_epoch=None, snapshot_path = None):
        ...
        with torch.no_grad():
            for i_batch, sampled_batch in enumerate(loader):
                ...
                # The model returns (logits, features) or (logits, features, logits_fake)
                model_output = model(image_batch)
                
                if isinstance(model_output, tuple):
                    if len(model_output) == 3:
                        outputs, emb, _ = model_output
                    else:
                        outputs, emb = model_output
                else:
