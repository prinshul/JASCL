...
#exp --method FT --name FT --lr ${lr} ${gen_par} --step 0 --debug --batch_size ${bs} --test
ns=5
is=0
inc_par="--ishot ${is} --input_mix novel --val_interval 50 --ckpt_interval 5 --no_pooling"

# vanilla 
exp --method FT --name FT --vanila --num_classes 21 --test --load_weight_strategy --epochs 10 --lr ${lr} ${gen_par} ${inc_par} --batch_size ${bs} --step 1 --debug --nshot ${ns} --step_ckpt ${path}/FT_0_newUNET_dynamic.pth
exp --method FT --name FT --vanila --num_classes 27 --test --load_weight_strategy --epochs 10 --lr ${lr} ${gen_par} ${inc_par} --batch_size ${bs} --step 2 --debug --nshot ${ns} --step_ckpt ${path}/FT_0_newUNET_dynamic.pth

#${path}/FT-s5-i0_1_newUNET_dynamic.pth
