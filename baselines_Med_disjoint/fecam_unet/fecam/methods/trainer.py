...
class FeCAM3D(nn.Module):
    def __init__(self, feature_dim, num_classes, use_covariance=True):
        super(FeCAM3D, self).__init__()
        self.num_classes = num_classes
        self.feature_dim = feature_dim
        self.prototypes = torch.zeros(num_classes, feature_dim).cuda()
        self.covariances = torch.eye(feature_dim).repeat(num_classes, 1, 1).cuda()
        self.use_covariance = use_covariance
    
    def forward(self, x):
        B, C, D, H, W = x.shape  # x - [B, 320, 16, 10, 6]
        x = x.permute(0, 2, 3, 4, 1).contiguous().view(-1, C)  
        distances = []
        for c in range(self.num_classes):
            diff = x - self.prototypes[c]
            if self.use_covariance:
                cov_inv = torch.inverse(self.covariances[c] + 1e-6 * torch.eye(self.feature_dim).cuda())
                maha_dist = torch.diag(diff @ cov_inv @ diff.T)
            else:
                maha_dist = torch.sum(diff ** 2, dim=1)
            distances.append(maha_dist)
        out = -torch.stack(distances, dim=1)  # Negative distance as similarity
        return out.view(B, D, H, W, self.num_classes).permute(0, 4, 1, 2, 3)  # [B, _, 16, 10, 6]
    
    def update_prototypes(self, features, labels):
        B, C, D, H, W = features.shape
        features = features.permute(0, 2, 3, 4, 1).contiguous().view(-1, C)
        ...
        for c in range(self.num_classes):
            ...
            if len(class_feats) > 0:
                ...
                if self.use_covariance:
                    self.covariances[c] = self._compute_covariance(class_feats)
    
    def _compute_covariance(self, features):
        return torch.cov(features.T) + 1e-6 * torch.eye(self.feature_dim).cuda()
...
class Trainer:
    def __init__(self, task, device, logger, opts, config_vit):
        ...
        num_classes = len(self.total_classes)+1 
        feature_dim = 320  # UNet Encoder output
        self.fecam = FeCAM3D(feature_dim, num_classes).cuda()
    ...
    def visualize_pred(self, labels, prediction, cur_epoch, cur_step):
        ...
        sitk.WriteImage(prediction_arr, os.path.join('merged/dataset/prediction/predicted_mask','{}_{}_{}.nii.gz'.format(cur_epoch, cur_step, self.opts.network_arch)))
    ...
    def train(self, optimizer, dice_weight, ce_weight, cur_epoch, train_loader, metrics=None, print_int=10, n_iter=1, snapshot_path = None):
        ...
        for i_batch, sampled_batch in enumerate(train_loader):
            ...
            outputs, fts = model(image_batch)
            #print(outputs.shape,fts.shape)
            #fecam_out = self.fecam(fts)
            #print(fecam_out.shape)
            #fecam_out = F.interpolate(outputs, size=(128, 160, 96), mode='trilinear', align_corners=False)
            #print(fecam_out.shape)
            #print(outputs[:,-fecam_out.shape[1]:].shape)
            #outputs[:,-fecam_out.shape[1]:]=fecam_out
    ...
    def test(self, dice_weight, ce_weight, loader, metrics, ret_samples_ids=None, novel=False, cur_epoch=None, snapshot_path = None):
        ...
        with torch.no_grad():
            for i_batch, sampled_batch in enumerate(loader):
                ...
                image_batch, label_batch = image_batch.cuda(), label_batch.cuda()
                # outputs, _ = model(image_batch)
                _, fts = model(image_batch)
                #print("Ground Truth", torch.unique(label_batch))
                #print(outputs.shape, fts.shape)
                
                fecam_out = self.fecam(fts)
                #print(fecam_out.shape)
                outputs = F.interpolate(fecam_out, size=(128, 160, 96), mode='trilinear', align_corners=False)
                #print(outputs.shape)
                #print(outputs[:,-fecam_out.shape[1]:].shape)
                #outputs[:,-fecam_out.shape[1]:]=fecam_out
