...
def step_zero_no_replay(opts,task,logger,rank,device):
    ...
    train_dst, val_dst,exemp_dst = get_dataset(opts, task, train=True)
    test_dst_all, test_dst_novel = get_dataset(opts, task, train=False)

    print("\n\nThe length of train set is: {}".format(len(train_dst)))
    #print("The length of exemplar set is: {}".format(len(exemp_dst)))
    ...
    #exemp_loader = DataLoader(exemp_dst, batch_size=batch_size, shuffle = True,
    ...
    #print("exemp_loader")
    #for i_batch, sampled_batch in enumerate(exemp_loader):
     #       print("Patient ids", sampled_batch['case_name'])
    #for (image, label) in enumerate(train_loader):
        #print(np.unique(label))
    #print("Concat loader")
    #for ci_batch, csampled_batch in enumerate(concat_loader):
     #       print("Patient ids", csampled_batch['case_name'])
    ...
    #logger.info(f"Concat loader contains {len(concat_loader)} iterations per epoch")
    logger.info(f"Total iterations are {opts.max_iter}, corresponding to {opts.epochs} epochs")
    
    
    # xxx Set up model
    logger.info(f"Backbone: {opts.network_arch}")
    ...
    #parameter_num, net = print_network(model.model)
    ...
    optimizer = torch.optim.AdamW(model.model.parameters(), lr=5e-4, eps=1e-8, betas=(0.9, 0.999), weight_decay=1e-2)
    ...
    #optimizer_df = torch.optim.AdamW(model.model.parameters(), lr=5e-4, eps=1e-8, betas=(0.9, 0.999), weight_decay=1e-2)
    
    # lr_scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, 44, eta_min=0, last_epoch=-1, verbose=False)          
    #lr_scheduler_df = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer, 25, eta_min=5e-6)
    ...
    print("\n\nTraining on base classes finished.")
...
def step_more_with_replay(opts,task,logger,rank,device):
    ...
    print('Subspace Reg will follow')
    task_name = f"{opts.task}-{opts.dataset}"
    name = f"{opts.name}-s{task.nshot}-i{task.ishot}" if task.nshot != -1 else f"{opts.name}"
    opts.step1_name = name
    ...
    train_loader = DataLoader(train_dst, batch_size=batch_size, shuffle = True,
                                       num_workers=opts.num_workers, pin_memory=pin_memory_flag,      # m_worker: 8    ---->>   4
                                 worker_init_fn=worker_init_fn)
    ...
    val_loader = DataLoader(val_dst, batch_size=batch_size, shuffle = True,
                                 num_workers=opts.num_workers, pin_memory=pin_memory_flag, worker_init_fn=worker_init_fn)
    ...
    #concat_dst =  torch.utils.data.ConcatDataset([train_dst,exemp_dst])
    
    #concat_loader = DataLoader(concat_dst, batch_size=batch_size, shuffle = True,
                                     #num_workers=opts.num_workers, pin_memory=True, worker_init_fn=worker_init_fn) 
    ...
    print(model)
    #summary(model,(1,32,opts.num_classes))
    #df_model = get_method(opts, task, device, logger, config_vit)
    # if opts.n_gpu > 1:
    #     model = torch.nn.DataParallel(model.model)
    ...
    if task.step > 0:
        ...
        assert step_ckpt is not None, "Step checkpoint is None!"
        # print(step_ckpt['model_state']["model"].keys())
        #print(step_ckpt['model_state']['model']['outc.weight'])
        #print(step_ckpt['model_state']['model']['outc.weight'].shape)
        #print(step_ckpt['model_state']['model']['outc.bias'].shape)
        base_ckpt=step_ckpt
        #base_weight, base_bias = model.get_base_weights(step_ckpt)
        #print(base_weight.shape, base_bias.shape)
        ...
        if opts.load_weight_strategy:
            ...
            ## This is running
            model.load_dict(old_model, strict=False)
            #df_model.load_dict(old_model, strict=False)
            print("model loaded from old model")
        print(f"[!] Previous model loaded from {step_ckpt['path']}")
        logger.info(f"[!] Previous model loaded from {step_ckpt['path']}")
        # clean memory
        del step_ckpt
    ...
    if not opts.vanila:
        if task.step > 0 and not opts.continue_ckpt and opts.ckpt is None:
            logger.info("Warm up lap!")
            model.warm_up(train_loader, old_model)
            #df_model.warm_up(train_loader, old_model)
            print("Model warm up")
    # put the model on DDP
    # model.distribute()
    ...
    #optimizer_df = torch.optim.AdamW(model.model.parameters(), lr=5e-4, eps=1e-8, betas=(0.9, 0.999), weight_decay=1e-5)
    ...
    #print(summary(model,(1,32,opts.num_classes)))
    ...
    #===================================== Training of Incremental Model ======================================================
    # train/val here
    while cur_epoch <= max_epoch and not opts.test:
        # =====  Train  =====
        start = time.time()
        class_dice_train, train_dice = model.train(optimizer, dice_weight, ce_weight, cur_epoch=cur_epoch,                     train_loader=train_loader, metrics=train_metrics, print_int=opts.print_interval,
                                 n_iter=train_iterations, snapshot_path = snapshot_path)
        # train_score = train_metrics.get_results()
        end = time.time()
    ...
    # ================================ Test code for Incremental Model============================================================
    #xxx Test code!
    logger.info("*** Test the model on all seen classes...")
    print("*** Test the model on all seen classes...")
