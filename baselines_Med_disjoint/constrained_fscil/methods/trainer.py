...
import random
import pickle
from .utils import get_batch, get_prototype_gaa
...
class Trainer:
    ...
    def make_model(self, config_vit, is_old=False):
        ...
        model = make_model(self.opts, config_vit, n_classes[0], classifier, 64)
        return model
    ...
    def get_classifier(self, is_old=False):
        ...
        if self.opts.method == "SPN":
            ...
            n_classes = self.task.get_n_classes()[:-1] if is_old else self.task.get_n_classes()
            cls = CosineClassifier(classes=n_classes, channels=64, cl=self.opts.num_classes)
        else:
            n_feat = self.opts.n_feat
            n_classes = self.task.get_n_classes()[:-1] if is_old else self.task.get_n_classes()
    ...
    def warm_up(self, dataset, epochs=1, bp =True):
        self.warm_up_(dataset, epochs, bipolarize=bp)
    ...
    def train(self, optimizer, dice_weight, ce_weight, cur_epoch, train_loader, metrics=None, print_int=10, n_iter=1, snapshot_path = None):
        ...
        pretrain_loss=torch.nn.NLLLoss()
        ...
        for i_batch, sampled_batch in enumerate(train_loader):
            ...
            if self.opts.step==0:
                embedding, features, logits = model(image_batch)
                #print("+++++++++++++++++Checkpoint 1++++++++++++++++")
            ...
            if iter_num % 1000 == 0:
                    self.visualize_pred(label_batch, logits, cur_epoch, iter_num)           
    ...
    def validate(self, dice_weight, ce_weight, loader, metrics, ret_samples_ids=None, novel=False, cur_epoch=None, snapshot_path = None):
        ...
        with torch.no_grad():
            for i_batch, sampled_batch in enumerate(loader):
                ...
                embedding, features, outputs = model(image_batch)
    ...
    def load_dict_imprint(self, path, strict=True, saving_gaa=False):
    ...
    def save_gaa(self, dataset, step):
      gaa={}
      if step>0:
         with open('gaa.pkl', 'rb') as f:
          gaa=pickle.load(f)
      ...
      print("Saving GAA for step {}, classes {}".format(step, classes))
      ...
      for c in classes:
          ...
          wc, bg = get_prototype_gaa(model, ds, c, self.device, interpolate_label=False, return_all=False, background=True)
          wc=wc.view(-1, 1, 1, 1)
          bg=bg.view(-1, 1, 1, 1)
      ...
      for key, val in gaa.items():
        print(key, val.shape)
      
      with open('gaa.pkl', 'wb') as f:
        pickle.dump(gaa, f)
      

    def replay_gaa(self, gaa, optimizer, ce_weight, me):
      device = self.device
      model = self.model
      num_classes = self.opts.num_classes
      class_weights = torch.FloatTensor(ce_weight).cuda()
      ...
      tot_loss=0
      print("Replaying GAA")
      print("Length of GAA : {}".format(len(gaa)))
      for cur_epoch in range(me):
        for label, feat in gaa.items():
            ...
            label=label.cuda()
            feat=feat.cuda()
            logits=model(feat, only_head=True)
            ...
            loss_ce.backward()
            optimizer.step()
        tot_loss/=num_classes
        print("Epoch: {} , Replay GAA CE Loss : {}".format(cur_epoch, tot_loss.item()))
