...
def make_model(opts, config_vit, n_classes, cls, head_channels):
    if opts.norm_act == 'abr':
        norm = partial(ABR, activation="relu")
    if opts.network_arch == 'shape_prior':    
        body = network(norm_act = norm, in_channel=3, out_channel=opts.num_classes, training=False, config=config_vit).cuda()
    elif opts.network_arch == 'newUNET':
        body = network(in_ch=1, base_ch=32, num_classes=opts.num_classes, block = 'SingleConv').cuda()
    ...
    head = nn.Conv3d(64, head_channels, kernel_size=1)
    ...
    def forward(self, x, use_classifier=True, return_feat=False, return_body=False,
        ...
        if only_classifier:
            ...
            if use_classifier:
                ...
                sem_logits = functional.interpolate(sem_logits, size=out_size, mode="trilinear", align_corners=False)
            else:
                sem_logits = out
            ...
            return x_b, out, sem_logits

    def freeze_extractor(self):
        for par in self.body.parameters():
            par.requires_grad = False
        for par in self.cls.cls.parameters():
            par.requires_grad = False
