...
class CosineClassifier(nn.Module):
    def __init__(self, classes, channels, cl):
        super().__init__()
        self.channels = channels
        self.cls = nn.Conv3d(channels, cl, 1, bias=False)
        self.scaler = 10.
        self.classes = classes
        self.tot_classes = 0
        for lcl in classes:
    ...
    def forward(self, inp):
        x = torch.tanh(inp)    
        kw=torch.tanh(self.cls.weight) #kernel weights activated by tanh

        x = F.normalize(x, p=2, dim=1)
        kw=F.normalize(kw, dim=1, p=2)

        similarity=F.conv3d(x, kw)
        #out = self.softabs()
        return self.scaler*similarity
    ...
    def softabs(self, x, steepness=10):
        act=torch.sigmoid(steepness * x) + torch.sigmoid(steepness -x) - 2
        t=F.normalize(act, dim=1, p=1)
        return t
