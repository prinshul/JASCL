...
def main(opts):
    ...
    if task.step > 0 and not opts.continue_ckpt and opts.ckpt is None:
        ...
        model.warm_up(train_dst, old_model, bp=True)
    ...
    optimizer = torch.optim.AdamW(model.model.parameters(), lr=5e-3, eps=1e-8, betas=(0.9, 0.999), weight_decay=1e-5)
    ...
    if opts.step>0:
        model.replay_gaa(gaa, optimizer, ce_weight, 400)
        logger.info("Classifier Alignment")
        print("Classifier Alignment")
        model.warm_up(train_dst, old_model, bp=False)
