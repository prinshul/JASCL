...
def train(args, model, start_epoch, best_acc):
    ...
    x_size, y_size = 512, 1024
    unlabeled_imgs = (NUM_CLASSES - NUM_CLASSES_OLD)*400
    total_pixels = (unlabeled_imgs*512*1024)*0.05
    per_batch_pixels = (total_pixels/(100/args.pseudo_label))/100 #2097152/100
    active_pixels = int(per_batch_pixels/4)
    print("According to 5% budget, per unlabeled image will have ",active_pixels," labels")
    
    mapper = HyperMapper(c=1.0)
    ...
    for epoch in range(start_epoch, args.num_epochs+1):
        ...
        for step, (images, labels) in enumerate(loader):
            ...
            '''
            out_conv = torch.argmax(outputs, axis=1).unsqueeze(1)
            print("out_conv ",out_conv.shape)
            
            region_uncertainty = compute_pixel_entropy(out_conv,NUM_CLASSES)
            #print(region_uncertainty)
            print(region_uncertainty.shape)
            
            
            embed = mapper.expmap(feats, dim=1)
            print(embed.shape)
             
            embed = F.interpolate(embed, size=input_shape, mode="bilinear", align_corners=True)
            print(embed.shape)
            
            region_impurity = mapper.poincare_distance_origin(
                embed, dim=1
            ).unsqueeze(1)
            
            count = torch.ones(
                (1, 1, 512, 1024), dtype=torch.float32
            ).cuda()
            
            print('region_impurity',region_impurity.shape)
            print('count',count.shape)
            
            prediction_uncertainty = region_uncertainty / count 
            print('prediction_uncertainty',prediction_uncertainty.shape)
            
            score = region_impurity * prediction_uncertainty
            
            print('score',score.shape)
            
            result_matrix = torch.zeros_like(score).cuda()
            
            for i in range(inputs.shape[0]):
                flat_indices = torch.argsort(score[i].view(-1),descending=True)
                top_indices = flat_indices[:active_pixels]
                rows, cols = torch.div(top_indices, y_size, rounding_mode='floor').long(), (top_indices % y_size).long()
                result_matrix[i, 0, rows, cols] = 1
            
            pick_labels = result_matrix == 1
            #pick_labels=pick_labels.cuda()
            out_conv[pick_labels] = targets[pick_labels]
            out_conv[pick_labels] = targets[pick_labels]
        ...
        if epoch%args.pseudo_label==0:
            print("Unlabeld data Active learning-training")
            for step_un, (images_un, labels_un) in enumerate(loader_unlabel):
                ...
                pick_labels = result_matrix == 1
                #pick_labels=pick_labels.cuda()
                targets_un[targets_un<(NUM_CLASSES_OLD-1)]=NUM_CLASSES-1
                out_conv[pick_labels] = targets_un[pick_labels]
                ...
                del outputs_un, out_conv, pick_labels, result_matrix, score, region_impurity, prediction_uncertainty
                # KLD on the output probability distributions of the teacher (outputs_prev_model) and student (outputs_prev_task)
...
if __name__ == '__main__':
    ...
    print("\n\nStep 2 Incremental : BDD->Cityscapes Few Shot Training\n\n")
