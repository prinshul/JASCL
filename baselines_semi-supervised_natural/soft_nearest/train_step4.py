...
#    global NUM_CLASSES, NUM_CLASSES_OLD
    ...
    for step, (images, labels) in enumerate(loader):
    				if args.cuda:
    								...
    								targets = labels.cuda()
    				with torch.no_grad():
    								outputs,fts = model(inputs)
    				print(fts.shape)
    				feature_list.append(torch.flatten(fts).unsqueeze(1).cpu())
    				label_list.append(torch.flatten(labels).unsqueeze(1).cpu())
...
def nearest_neighbour_label(args, model, co_transform_val,file_dir,file_list,ft_list,label_list, to_copy_path):
#def extract_features(args, model, loader):	
#def train(args, model, model_old, start_epoch, best_acc):
#    global NUM_CLASSES, NUM_CLASSES_OLD
    feature_list = []
    
    image_path = to_copy_path + "train/images/"
    lbl_path = to_copy_path + "train/labels/"
    os.makedirs(to_copy_path,exist_ok=True)
    os.makedirs(to_copy_path+"train/",exist_ok=True)
    os.makedirs(image_path,exist_ok=True)
    os.makedirs(lbl_path,exist_ok=True)
    
    ft_list=ft_list.cuda()     
    k=0
    for files in file_list:
    				if k==100: break
    				file_path = file_dir+"images/"+files.replace("gtFine_labellevel3Ids","leftImg8bit")
    				img_read = Image.open(file_path).convert('RGB')
    				lbl_read = Image.open(file_path).convert('RGB')
    				
    				pred_labels = torch.zeros(512*1024).cuda()
    				
    				image,label = co_transform_val(img_read,lbl_read)
    				
    				with torch.no_grad():
    								_,img_ft = model(image.unsqueeze(0).cuda())
    				img_ft = torch.flatten(img_ft).unsqueeze(1)
    				
    				for i in range(len(pred_labels)):
    								distances = torch.linalg.norm(ft_list[i] - img_ft, dim=0)
    								nearest_idx = torch.argmin(distances)
    								pred_labels[i] = label_list[i][nearest_idx]
    				#print(torch.unique(pred_labels))
    				pred_labels_np = Image.fromarray(torch.reshape(pred_labels, (512,1024)).cpu().numpy().astype(np.uint8))
    				k+=1
         
        
    				pred_labels_np.save(lbl_path+files)
    				shutil.copy(file_path,image_path)  
    				sys.stdout.flush()
    print('Nearest neighbours labelling complete')
    sys.stdout.flush()
...
def train(args, model, model_old, start_epoch, best_acc):
    ...
    loader_train = DataLoader(dataset_step3_train, num_workers=args.num_workers,
                        batch_size=1, shuffle=False)
    
    

    ft_list, label_list = extract_features(args,model,loader_train)
    print("Feature list and label list shape", ft_list.shape,label_list.shape)
    sys.stdout.flush()
    
    unlabel_dir = 'data_used/step4/'
    nearest_neighbour_label(args, model, co_transform_val,camvid_datadir+"train/",step3_idd_unlabel_files,ft_list,label_list, unlabel_dir)
    torch.cuda.empty_cache()
    unlabeled_files = os.listdir(unlabel_dir+"train/labels/")
    dataset_unlabel = Camvid(unlabel_dir,unlabeled_files, co_transform_val, 'train')
    ...
    for epoch in range(start_epoch, args.num_epochs+1):
        ...
        for step, (images, labels) in enumerate(loader):
            ...
            with torch.no_grad():
                ...
                outputs_prev_model,_ = model_old(inputs)
        ...
        if epoch%args.pseudo_label==0:
            ...
            for step_un, (images_un, labels_un) in enumerate(loader_unlabel):
                ...
                if args.cuda:
                    ...
                    labels_un = labels_un.cuda()
                outputs_un,_ = model(inputs_un)
                
                
                
                with torch.no_grad():
                # pass same input through the old model as it is, calc KLD as KD between old CS and new CS ; and backprop only thru the enc shared weights.
                    outputs_prev_model,_ = model_old(inputs_un)

                ce_loss = criterion(outputs_un, labels_un[:,0])  # cross entropy, main classification loss
                ce_loss = ce_loss.mean() 
        ...
        average_loss_val, val_acc = eval( args, model, 
         model_old, loader_val, criterion, kl_loss, current_task, args.num_classes, epoch)
...
def eval(args, model, model_old, dataset_loader, criterion, kl_loss, task, num_classes, epoch):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset_old', nargs="+", help='pass list of datasets in order',
                        required=False, default=['BDD_step1','Cityscape','IDD','Camvid'])
    ...
    print("\n",os.getcwd())
    print("\n\nStep 4 Incremental : BDD->Cityscape->IDD->Camvid Training\n")
