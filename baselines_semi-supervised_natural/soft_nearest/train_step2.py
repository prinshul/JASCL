...
#    global NUM_CLASSES, NUM_CLASSES_OLD
    ...
    for files in file_list:
    				if k%100==0: print(k," images processed")
    				file_path = file_dir+"images/"+files.replace("gtFine_labelIds","leftImg8bit")
...
def train(args, model, model_old, start_epoch, best_acc):
    ...
    weight_IDD = weights_step2
    weight_IDD[NUM_CLASSES - 1] = 0    
    ...
    if data_name == 'cityscape':
        print('taking cityscapes')
        dataset_train = cityscapes(city_datadir,train_gt_files, co_transform, 'train')
        dataset_val = cityscapes(city_datadir,val_gt_files, co_transform_val, 'train')
    ...
    unlabel_dir = 'data_used/step2/'
    #nearest_neighbour_label(args, model, co_transform_val,city_datadir+"train/",unlabeled_files,ft_list,label_list, unlabel_dir)
    
    unlabeled_files = os.listdir(unlabel_dir+"train/labels/")
    dataset_unlabel = cityscapes(unlabel_dir,unlabeled_files, co_transform_val, 'train')
    ...
    if args.cuda:
        class_weights = weight_IDD.to(device)
    ...
    for epoch in range(start_epoch, args.num_epochs+1):
        ...
        for step, (images, labels) in enumerate(loader):
            ...
            #out_conv = torch.argmax(outputs, axis=1)
            #print('out_conv',out_conv.shape)
            #print(torch.unique(out_conv))
            #print(torch.unique(out_conv.long()))
            #sys.exit()
...
def main(args):
    ...
    if args.eval_type=='train':
        ...
        if args.state:
            savedir_prev_step = 'save/step{}/'.format(current_task)
