...
class Trainer:
        ...
        for i_batch, sampled_batch in enumerate(train_loader):
            ...
            if not self.opts.vanila:
                if self.model_old is not None:
                    outputs_old = self.model_old(image_batch)
                    if self.kd_criterion is not None:
                        #print("kd between output")
    ...
    def pseudo_train(self, optimizer, dice_weight, ce_weight, cur_epoch, train_loader, metrics=None, print_int=10, n_iter=1, snapshot_path = None):
        ...
        for i_batch, sampled_batch in enumerate(train_loader):
            ...
            # print("Prediction", torch.unique(outputs))
            pseudo_out = torch.argmax(outputs,dim=1)
            loss_ce_1 = ce_loss(outputs, pseudo_out[:].long())
            class_freq, class_wise_dice, loss_dice_1 = dice_loss(outputs, pseudo_out[:].long(), self.total_classes, softmax=True)
