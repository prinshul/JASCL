...
def get_dataset(opts, task, train=True):
    ...
    if train:
        if opts.cross_val:
            ...
            val_dst = dataset(root=opts.data_root, task=task, base_dir=opts.val_root_path, train="val", 
                            num_classes=opts.num_classes,
            ...
            exemp_dst = dataset(root=opts.data_root, task=task, base_dir=opts.train_root_path, train="exemplar", 
                                num_classes=opts.num_classes,
                                   transform=transforms.Compose([RandomGenerator(output_size=opts.img_size, mode = 'val')]))
            
            exemp_dst2 = dataset(root=opts.data_root, task=task, base_dir=opts.train_root_path, train="exemplar2", 
                                num_classes=opts.num_classes,
                                   transform=transforms.Compose([RandomGenerator(output_size=opts.img_size, mode = 'val')]))
            
        return train_dst, val_dst , exemp_dst, exemp_dst2         
