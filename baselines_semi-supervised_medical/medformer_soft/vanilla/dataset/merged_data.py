...
class MergedSegmentation(data.Dataset):
    ...
    def __init__(self, root, base_dir, split, num_classes, transform=None):
        ...
        exemp_path = "semi_fscil/medical/unlabeled/merged"
        self.exemp_path = exemp_path
        #print("root, data_dir",root,self.data_dir)

        ds_root = self.data_dir #osp.join(root, self.data_dir)
        print("ds_root:", ds_root, split)
        splits_dir = osp.join(ds_root, 'split')
        print(splits_dir)
        
        if self.split == 'exemplar' or self.split == 'exemplar2':
            self.sample_list = open(os.path.join(self.exemp_path+"/split", self.split + '.txt')).readlines()
            print("For exemplar ",os.path.join(self.exemp_path+"/split", self.split + '.txt'))
        ...
        if self.split == 'train':
            ...
            self.class_to_images_ = pkl.load(open(splits_dir + path, 'rb'))
        elif self.split == 'exemplar' or self.split == 'exemplar2':
            path = '/inverse_dict_new_exemplar.pkl'
            self.class_to_images_ = pkl.load(open(self.exemp_path+"/split" + path, 'rb'))
        else:
        ...
        step1_file = open(os.path.join(splits_dir, 'step1.txt')).readlines()
        step2_file = open(os.path.join(splits_dir, 'step2.txt')).readlines()
        step3_file = open(os.path.join(splits_dir, 'step3.txt')).readlines()
        step4_file = open(os.path.join(splits_dir, 'step4.txt')).readlines()
        step5_file = open(os.path.join(splits_dir, 'step5.txt')).readlines()


        if self.split == 'train':

            if self.num_classes==21:
                rem_files=step1_file
            if self.num_classes==27:
                rem_files=step2_file
            if self.num_classes==31:
                rem_files=step3_file
            if self.num_classes==34:
                rem_files=step4_file
            if self.num_classes==38:
                rem_files=step5_file
            
            keep_nums= []
            for i in range(0,len(self.sample_list)):
                if self.sample_list[i] in rem_files:
                    keep_nums.append(i)
            print(keep_nums)
            print("To keep = ",len(keep_nums))


            for key in self.class_to_images_.keys():
                curr_arr=self.class_to_images_[key]
                not_keep = list(filter(lambda x: x not in set(keep_nums),curr_arr))
                for numb in not_keep:
                    if numb in self.class_to_images_[key]:
                        self.class_to_images_[key].remove(numb)   
        ...
        #print(self.sample_list)
        #print(self.images_to_idx_)
        #print(self.class_to_images_)
    ...
    def __getitem__(self, idx):
        if self.split == "train":
            ...
            if "BraTS20" in slice_name:
                img_path = os.path.join(self.data_dir, 'dataset', 'images', 'train', slice_name + '.nii')
                label_path = os.path.join(self.data_dir, 'dataset', 'annotations', 'train', slice_name + '.nii')
            else:
        ...
        elif self.split == "exemplar":
            
            slice_name = self.sample_list[idx].strip('\n')
            if "BraTS20" in slice_name:
                img_path = os.path.join(self.exemp_path, 'dataset', 'images', 'train', slice_name + '.nii')
                label_path = os.path.join(self.exemp_path, 'dataset', 'annotations', 'train', slice_name + '.nii')
            else:
                img_path = os.path.join(self.exemp_path, 'dataset', 'images', 'train', slice_name + '.nii.gz')
                label_path = os.path.join(self.exemp_path, 'dataset', 'annotations', 'train', slice_name + '.nii.gz')
            ...
            # #print(np.unique(label, return_index=True))
            # print("labels2:", np.unique(label, return_index=True))
        elif self.split == "exemplar2":
            slice_name = self.sample_list[idx].strip('\n')
            data_fetch_from = "semi_fscil/medical/medformer_soft/vanilla/data_used/"
            if self.num_classes==21:
                rem_files_path= data_fetch_from+"step1/"
            if self.num_classes==27:
                rem_files_path= data_fetch_from+"step2/"
            if self.num_classes==31:
                rem_files_path= data_fetch_from+"step3/"
            if self.num_classes==34:
                rem_files_path= data_fetch_from+"step4/"
            if self.num_classes==38:
                rem_files_path= data_fetch_from+"step5/"

            print("Fetched slice ",slice_name," files from ",rem_files_path)
            if "BraTS20" in slice_name:
                img_path = os.path.join(rem_files_path,'images', slice_name + '.nii')
                label_path = os.path.join(rem_files_path,'annotations', slice_name + '.nii')
            else:
                img_path = os.path.join(rem_files_path,'images', slice_name + '.nii.gz')
                label_path = os.path.join(rem_files_path,'annotations', slice_name + '.nii.gz')
            ...
            slice_name = self.sample_list[idx].strip('\n')
            if "BraTS20" in slice_name:
                img_path = os.path.join(self.data_dir, 'dataset', 'images', 'val', slice_name + '.nii')
                label_path = os.path.join(self.data_dir, 'dataset', 'annotations', 'val', slice_name + '.nii')
            else:
            ...
            slice_name = self.sample_list[idx].strip('\n')
            if "BraTS20" in slice_name:
                img_path = os.path.join(self.data_dir, 'dataset', 'images', 'test', slice_name + '.nii')
                label_path = os.path.join(self.data_dir, 'dataset', 'annotations', 'test', slice_name + '.nii')
            else:
