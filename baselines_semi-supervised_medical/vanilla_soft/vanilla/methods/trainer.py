...
class Trainer:
    def __init__(self, task, device, logger, opts, config_vit):
       ...
       # Feature distillation
        ...
        if opts.loss_kd > 0 or opts.mib_kd > 0:
            ...
            if opts.loss_kd > 0:
                if opts.ckd:
                    print("Using cosine KD", opts.loss_kd)
    ...
    def extract_features(self, loader):	
        ...
        for i_batch, sampled_batch in enumerate(loader):
            ...
            with torch.no_grad():
                output,fts = model(image_batch)
            del output

            feature_list.append(torch.flatten(fts[:,:1,:]).unsqueeze(1).cpu())
            label_list.append(torch.flatten(label_batch).unsqueeze(1).cpu())
    ...
    def nearest_neighbour_label(self, loader,ft_list,label_list, to_copy_path):
        
        file_path = 'med_3D/dataset/set1_extra/set1_full/merged/dataset/images/train/'
        
        model = self.model
        image_path = to_copy_path + "images/"
        lbl_path = to_copy_path + "annotations/"
        ...
        k=0
        for i_batch, sampled_batch in enumerate(loader):
            file_name = sampled_batch['case_name'][0]+".nii.gz"
            print("Processing ..",file_name)
            if os.path.isfile(lbl_path+file_name): continue 

            image_batch, label_batch = sampled_batch['image'], sampled_batch['label']
            image_batch = image_batch.cuda()
            
            pred_labels = torch.zeros(128*160*96).cuda()
            
            with torch.no_grad():
                _,img_ft = model(image_batch)

            img_ft = torch.flatten(img_ft[:,:1,:]).unsqueeze(1)
            ...
            pred_labels = torch.reshape(pred_labels,(128,160,96)).cpu().numpy()
            pred_labels_np = sitk.GetImageFromArray(pred_labels)
            sitk.WriteImage(pred_labels_np,lbl_path+file_name)
            
            img_reshape = torch.reshape(image_batch,(128,160,96)).cpu().numpy()
            img_np = sitk.GetImageFromArray(img_reshape)
            sitk.WriteImage(img_np,image_path+file_name)
            
            #pred_labels_np.save(lbl_path+file_name)
            #shutil.copy(file_path+file_name,image_path)  
    ...
    def train(self, optimizer, dice_weight, ce_weight, cur_epoch, train_loader, metrics=None, print_int=10, n_iter=1, snapshot_path = None):
        ...
        for i_batch, sampled_batch in enumerate(train_loader):
            ...
            #pseudo_out = torch.argmax(outputs,dim=1)
            ...
            if not self.opts.vanila:
                if self.model_old is not None:
                    outputs_old, _ = self.model_old(image_batch)
            ...
            rest_loss_values=(1.0-self.kd_loss)/2.0
            
            
            #print("check = ",rest_loss_values, rloss)
    ...
    def pseudo_train(self, optimizer, dice_weight, ce_weight, cur_epoch, train_loader, metrics=None, print_int=10, n_iter=1, snapshot_path = None):
        """Train and return epoch loss"""
        if metrics is not None:
            metrics.reset()
        logger = self.logger
        optim = optimizer
        print("Exemp loader 2 length = ",len(train_loader))
        logger.info("Epoch %d, lr = %f" % (cur_epoch, optim.param_groups[0]['lr']))
