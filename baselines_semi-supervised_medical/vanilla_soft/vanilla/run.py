...
def main(opts):
    ...
    train_dst, val_dst, exemp_dst, exemp_dst2 = get_dataset(opts, task, train=True)
    ...
    exemp_loader = DataLoader(exemp_dst, batch_size=1, shuffle = True,
                                 num_workers=opts.num_workers, pin_memory=True, worker_init_fn=worker_init_fn)                             
    ...
    print("Doing nearest neighbout labelling using model")
    sys.stdout.flush()
    save_exemp_path = "data_used/step{}/".format(opts.step)
    os.makedirs(save_exemp_path,exist_ok=True)
    ft_list, label_list = model.extract_features(exemp_loader)
    model.nearest_neighbour_label(exemp_loader,ft_list,label_list,save_exemp_path)
    ...
    exemp_loader2 = DataLoader(exemp_dst2, batch_size=batch_size, shuffle = True,
                                 num_workers=opts.num_workers, pin_memory=True, worker_init_fn=worker_init_fn)          
    print()
    print("exemp_loader2 processed")
    for i_batch, sampled_batch in enumerate(exemp_loader2):
            print("Patient ids", sampled_batch['case_name'])
            image_batch, label_batch = sampled_batch['image'], sampled_batch['label']
            print(image_batch.shape,label_batch.shape)
            print(np.unique(label_batch))
    ...
    while cur_epoch <= max_epoch and not opts.test:
        ...
        if cur_epoch % 25 == 0 and cur_epoch>0:
            print("*** Train the model on unlabeled samples...")
            pseudo_train = model.pseudo_train(optimizer, dice_weight, ce_weight, cur_epoch=cur_epoch, train_loader=exemp_loader2,
                                 metrics=train_metrics, print_int=opts.print_interval,
                                 n_iter=train_iterations, snapshot_path = snapshot_path)
    ...
    test_dst_all, test_dst_novel,_ = get_dataset(opts, task, train=False)
    print("The length of test set is: {}".format(len(test_dst_all)))
