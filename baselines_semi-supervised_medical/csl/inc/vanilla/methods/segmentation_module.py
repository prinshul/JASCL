...
from networks.med_former.medformer import MedFormer 
from networks.med_former import med_args
...
def make_model(opts, config_vit, n_classes):
    ...
    elif opts.network_arch == 'medformer':
        
        model = MedFormer(in_chan=med_args.in_chan, num_classes=opts.num_classes, map_size=med_args.map_size, \
        conv_block=med_args.conv_block, conv_num=med_args.conv_num, trans_num=med_args.trans_num, \
                  num_heads=med_args.num_heads, fusion_depth=med_args.fusion_depth, fusion_dim=med_args.fusion_dim,\
                  fusion_heads=med_args.fusion_heads, \
                  expansion=med_args.expansion, attn_drop=med_args.attn_drop, proj_drop=med_args.proj_drop,\
                  proj_type=med_args.proj_type, norm=med_args.norm, \
                  act=med_args.act, kernel_size=med_args.kernel_size, scale=med_args.down_scale,\
                  aux_loss=med_args.aux_loss, curr_step=opts.step).cuda()
    return model
