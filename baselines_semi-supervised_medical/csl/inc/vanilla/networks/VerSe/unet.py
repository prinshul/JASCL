...
class network(nn.Module):
    def __init__(self, norm_act=nn.BatchNorm3d, in_channel=3, out_channel=2, training=True, config=None):
        ...
        self.encoder1 = nn.Sequential(
            Conv3dReLU(norm_act, in_channel, channels[0], kernel_size=3, padding=1),
            Conv3dReLU(norm_act, channels[0], channels[0], kernel_size=3, padding=1)
        )
        
        self.decoder1 = nn.Sequential(
            Conv3dReLU(norm_act, self.dim + self.skip_channels[0], self.decoder_channels[0], kernel_size=3, padding=1),
            Conv3dReLU(norm_act, self.decoder_channels[0], self.decoder_channels[0], kernel_size=3, padding=1)
        )
        self.decoder2 = nn.Sequential(
            Conv3dReLU(norm_act, self.decoder_channels[0] + self.skip_channels[1], self.decoder_channels[1], kernel_size=3, padding=1),
            Conv3dReLU(norm_act, self.decoder_channels[1], self.decoder_channels[1], kernel_size=3, padding=1)
        )
        self.decoder3 = nn.Sequential(
            Conv3dReLU(norm_act, self.decoder_channels[1] + self.skip_channels[2], self.decoder_channels[2], kernel_size=3, padding=1),
            Conv3dReLU(norm_act, self.decoder_channels[2], self.decoder_channels[2], kernel_size=3, padding=1)
        )  # b, 1, 28, 28
        self.decoder4 = nn.Sequential(
            Conv3dReLU(norm_act, self.decoder_channels[2] + channels[0], channels[0], kernel_size=3, padding=1),
            Conv3dReLU(norm_act, channels[0], channels[0], kernel_size=3, padding=1)
        )
        ...
        # self.linear_layer = nn.Linear(channels[0] * 128 * 160 * 96, channels[0]*27)  # Modify the input and output dimensions as needed
        # self.feature = nn.Conv3d(channels[0], channels[0], kernel_size=7, padding=0, stride=(42, 53, 32))
        # self.feature = nn.Conv3d(in_channels=16, out_channels=21, kernel_size=(5, 7, 5), padding=(0, 0, 0), stride=(15, 20, 15))
        self.feature = nn.Conv3d(in_channels=channels[0], 
    ...
    def forward(self, x):
        ...
        # feature_extractor = self.linear_layer(x.view(x.size(0), -1).clone())
    ...
    def extract(self, x):
            ...
            x = torch.cat((x, t1), 1)
            x = self.decoder4(x)
            # x = self.linear_layer(x.view(x.size(0), -1).clone())
            # x = x.view(x.size(0), 16, 27)
            x = self.feature(x)
