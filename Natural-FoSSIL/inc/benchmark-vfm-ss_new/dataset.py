...
class Dataset(torch.utils.data.Dataset):
    def __init__(
        ...
        img_folder_path_in_zip: Path = None,
        target_folder_path_in_zip: Path = None,
        ...
        self.img_folder_path_in_zip = img_folder_path_in_zip
        self.target_folder_path_in_zip = target_folder_path_in_zip
        
        self.ignore_idx = ignore_idx
        self.transforms = transforms
        self.class_mapping = class_mapping
        self.is_train_file = is_train_file
        ...
        self.imgs = []
        self.targets = []
        #print(self.img_folder_path_in_zip)
        
        if self.step_data==0:
            for files in self.is_train_file:
                self.imgs.append(Path(self.img_folder_path_in_zip,files[:-3]+"jpg"))
                self.targets.append(Path(self.target_folder_path_in_zip,files))
        if self.step_data==1:
            for files in self.is_train_file:
                self.imgs.append(Path(self.img_folder_path_in_zip,files.split("/")[-1].replace("gtFine_labellevel3Ids","leftImg8bit")))
                self.targets.append(Path(self.target_folder_path_in_zip,files.split("/")[-1]))
        
        print("length of data ",len(self.imgs),len(self.targets))
    ...
    def __getitem__(self, index: int):
        ...
        # Handle case where no valid masks are found
        if len(masks) == 0:
            # Create a dummy mask and label for the ignore class
            # This ensures we always have at least one mask/label pair
            dummy_mask = torch.zeros_like(target, dtype=torch.bool)
            masks.append(dummy_mask)
            labels.append(torch.tensor([self.ignore_idx if self.ignore_idx is not None else 0]))
