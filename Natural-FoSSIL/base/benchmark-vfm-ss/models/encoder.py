...
class Encoder(nn.Module):
    def __init__(
        ...
        if ckpt_path:
            print("Taking previous model from ",ckpt_path)
            checkpoint_load = torch.load(ckpt_path)
            #print(checkpoint_load)
            print("Previous model epoch : ",checkpoint_load['epoch'])
            print("Previous model global_step : ",checkpoint_load['global_step'])
            print("Previous model pytorch-lightning_version : ",checkpoint_load['pytorch-lightning_version'])
            
            prev_state_dict = checkpoint_load['state_dict']
            print(type(prev_state_dict))
            #for keys in checkpoint_load['state_dict']:
                #print(keys)
            ...
            for key in prev_state_dict:
                if "head" not in key:
                    if "pixel_mean" not in key:
                        if "pixel_std" not in key:
                            if "rel_pos_h" not in key:
                                if "rel_pos_w" not in key:
                                    new_key = key.replace("network.","")
                                    new_key = new_key.replace("encoder.","")
                                    new_state[new_key]=prev_state_dict[key]
                ...
                if "head" not in key:
                    if "pixel_mean" not in key:
                        if "pixel_std" not in key:
                            new_key = key.replace("network.","")
                            new_key = new_key.replace("encoder.","")
                            curr_model_sh = self.encoder.new_key
                            new_state[new_key]=prev_state_dict[key]
            '''
