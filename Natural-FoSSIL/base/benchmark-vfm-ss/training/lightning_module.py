...
class LightningModule(lightning.LightningModule):
    def __init__(
        ...
        print("\n\nEncoder params , freeze = ", freeze_encoder)
    ...
    def forward(self, imgs):
        ...
        #print("Input x ",x.shape) torch.Size([1, 3, 1024, 1024])
        output, feat = self.network(x)
        #if self.k==1:
        #    print(self.network)
        #    self.k+=1
        #print("self.network(x) ",self.network(x).shape) torch.Size([1, 15, 64, 64])
        #with torch.no_grad():
        #    feat = self.network.encoder(x) ## (1,768)
        #    out_c = self.network.head(feat)
        #   print("output, feat", out_c.shape, feat.shape)
            
        #print("lightning module output, feat", output.shape, feat.shape)
        #lightning module output, feat torch.Size([1, 15, 64, 64]) torch.Size([1, 768, 64, 64])
    ...
    def _on_eval_epoch_end_semantic(self, log_prefix):
        print(f"Computing metrics for {len(self.metrics)} metric modules...")
        miou_per_dataset = []
        iou_per_dataset_per_class = []
        
        for metric_idx, metric in enumerate(self.metrics):
            print(f"Computing metric {metric_idx}...")
            iou_per_dataset_per_class.append(metric.compute())
            print(f"Metric {metric_idx} computed: {iou_per_dataset_per_class[-1]}")
            metric.reset()

            for iou_idx, iou in enumerate(iou_per_dataset_per_class[-1]):
                self.log(f"{log_prefix}_{metric_idx}_iou_{iou_idx}", iou, sync_dist=True)

            miou_per_dataset.append(float(iou_per_dataset_per_class[-1].mean()))
            print(f"mIoU for dataset {metric_idx}: {miou_per_dataset[-1]:.4f}")
            self.log(f"{log_prefix}_{metric_idx}_miou", miou_per_dataset[-1], sync_dist=True)
            sys.stdout.flush()
