...
def main(opts):
    ...
    print("STREAMSEG", task.get_n_classes()[0])
    ...
    while cur_epoch < max_epoch and not opts.test:
        ...
        if cur_epoch % 50 == 0:
            ...
            with open('/hdd2/cil/no_class_incremental_ablation/results/dice_step_{}_{}_test_{}_dynamic.txt'.format(task.step, cur_epoch, opts.network_arch), 'w') as file:
    ...
    with open('/hdd2/cil/no_class_incremental_ablation/results/dice_step_{}_train_{}_dynamic.txt'.format(task.step, opts.network_arch), 'w') as file:
        for key, value in class_wise_dice_train_dict.items():
            file.write(f'{key}: {value}\n')
        
    with open('/hdd2/cil/no_class_incremental_ablation/results/dice_step_{}_val_{}_dynamic.txt'.format(task.step, opts.network_arch), 'w') as file:
    ...
    with open('/hdd2/cil/no_class_incremental_ablation/results/dice_step_{}_test_{}_dynamic.txt'.format(task.step, opts.network_arch), 'w') as file:
