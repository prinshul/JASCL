...
class Trainer:
    ...
    def train(self, optimizer, dice_weight, ce_weight, cur_epoch, train_loader, metrics=None, print_int=10, n_iter=1, snapshot_path = None):
        ...
        for i_batch, sampled_batch in enumerate(train_loader):
            ...
            if self.opts.step==0:           
                ...
                # # print(label_shape)
                # # print(img_shape)
                # image_batch = torch.stack([torch.rot90(image_batch, k, (3, 4)) for k in range(4)], 1)
                # image_batch=image_batch.view(-1, img_shape[1], img_shape[2], img_shape[3], img_shape[4])

                # label_batch = torch.stack([torch.rot90(label_batch, k, (2, 3)) for k in range(4)], 1)
                # label_batch=label_batch.view(-1, label_shape[1], label_shape[2], label_shape[3])
                ...
                # protos = get_proto_list_old()
                # indexes = torch.randperm(protos.shape[0])
                
                # protos = protos[indexes]
                # protos=protos.view(protos.shape[0], temp_protos.shape[1], 1, 1, 1)
                ...
                # logits_protos=logits_protos.view()

                # loss_proto = nn.CrossEntropyLoss()(logits_protos, label_proto) * args.lamda_proto    
                feat, logits = model(image_batch)
    ...
    def filter_pseudo_labels(self, logits, features, proto_dict, confidence_thresh=0.7, similarity_thresh=0.7, metric='cosine'):
        """
        Filters pseudo-labels based on confidence and prototype alignment.

        Args:
            logits: Tensor of shape (B, C, H, W, D) - predicted logits from the model.
            features: Tensor of shape (B, D_feat, H, W, D) - feature map from decoder.
            prototypes: Tensor of shape (C, D_feat) - class prototypes.
            confidence_thresh: float - minimum softmax confidence to consider a pseudo-label.
            similarity_thresh: float - minimum similarity (cosine or inverse distance) to keep a label.
            metric: 'cosine' or 'euclidean' - distance metric for prototype alignment.

        Returns:
            filtered_pseudo_labels: Tensor of shape (B, H, W, D) - with ignored regions set to -1.
        """
        B, C, H, W, D = logits.shape
        _, D_feat, _, _, _ = features.shape

        # Softmax over logits to get class probabilities
        probs = F.softmax(logits, dim=1)
        max_probs, pseudo_labels = torch.max(probs, dim=1)  # (B, H, W, D)

        # Reshape for computation: (B*H*W*D, D_feat)
        features_flat = features.permute(0, 2, 3, 4, 1).reshape(-1, D_feat)
        labels_flat = pseudo_labels.view(-1)
        max_probs_flat = max_probs.view(-1)
        ...
        filtered_labels_flat[~keep_mask] = -1

        # Step 7: Reshape to original label map
        filtered_lowres_labels = filtered_labels_flat.view(B, H, W, D).float() 

        #filtered_pseudo_labels = torch.clamp(filtered_lowres_labels,0,prototypes.shape[0])
        
        return filtered_lowres_labels
    ...
    def pseudo_train(self, optimizer, dice_weight, ce_weight, cur_epoch, train_loader, labeled_loader, metrics=None, print_int=10, n_iter=1, snapshot_path = None):
        """Train and return epoch loss"""
        if metrics is not None:
            metrics.reset()
        logger = self.logger
        ...
        for i_batch, sampled_batch in enumerate(train_loader):
            ...
            pseudo_out = self.filter_pseudo_labels(outputs_st, features_st, protos)
                
            features_tea, outputs_tea = self.teacher(image_batch)
            pseudo_out_tea = self.filter_pseudo_labels(outputs_tea, features_tea, protos)
            
            ## Consistency loss
            mse_loss = F.mse_loss(pseudo_out_tea,pseudo_out)
            
            del pseudo_out_tea,features_tea,outputs_tea,pseudo_out,features_st, outputs_st
            del image_batch, label_batch 
            ...
            #print(outputs.shape, lbl_image_batch.shape, lbl_label_batch.shape)
            loss_ce_1 = ce_loss(outputs, lbl_label_batch[:].long())
            class_freq, class_wise_dice, loss_dice_1 = dice_loss(outputs, lbl_label_batch[:].long(), self.total_classes, softmax=True)

            loss_ce = loss_ce_1
            loss_dice = loss_dice_1

            
            loss = loss_ce + loss_dice + 0.05*mse_loss
        ...
        logger.info('Unlabeled epoch : %d, mean train loss : %f, mean train ce loss: %f, mean train dice : %f' % (cur_epoch, mean_loss, mean_loss_ce, mean_dice))
        print('Unlabeled epoch : %d, mean train loss : %f, mean train ce loss: %f, mean train dice : %f' % (cur_epoch, mean_loss, mean_loss_ce, mean_dice))
        sys.stdout.flush()
