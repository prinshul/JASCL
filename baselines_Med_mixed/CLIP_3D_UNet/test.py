...
def validation(model, ValLoader, val_transforms, args):
    save_dir = 'pred_results/' + args.log_name + f'/test_healthp_{args.epoch}'
    if not os.path.isdir(save_dir):
        os.makedirs('pred_results/' + args.log_name, exist_ok=True)
    ...
    for index, batch in enumerate(tqdm(ValLoader)):
        ...
        #pred_hard = threshold_organ(pred_sigmoid, organ=ORGAN_NAME, threshold=THRESHOLD_DIC)
        ...
        for b in range(B):
            ...
            template_key = args.data_txt_path         #get_key(name[b])
    ...
    with open('pred_results/'+args.log_name+f'/test_{args.epoch}_{args.data_txt_path}.txt', 'w') as f:
        for key in TEMPLATE.keys():
            organ_list = TEMPLATE[key]
            content = 'Task %s| '%(key)
            for organ in organ_list:
                dice = dice_list[key][0][organ-1] / dice_list[key][1][organ-1]
                content += '\n%s: %.4f, '%(ORGAN_NAME[organ-1], dice)
...
def main():
    ...
    ## model load
    parser.add_argument('--resume', default='./pretrained_weights/unet.pth', help='The path resume from checkpoint')
    parser.add_argument('--pretrain', default='./pretrained_weights/unet.pth', 
    ...
    word_embedding_path = "pretrained_weights/txt_encoding_"+str(NUM_CLASS)+".pth"
    ...
    print("Checkpoint from ",args.resume)
