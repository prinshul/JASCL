...
def train(args, model, start_epoch, best_acc):
    ...
    with open(shots_datadir+'nshot_'+nshot+'/city_unlabel_step2.pkl', 'rb') as file: 
        unlabeled_files=pickle.load(file)
    
    print("\n\nMaking GAPS data for Step 2 : pasting on Step 1 base data ")
    curr_path = 'data_used/step2/'#image_paste_step2(train_gt_files,val_gt_files) 
    ...
    if data_name == 'cityscape':
        ...
        dataset_unlabel = cityscapes(city_datadir,unlabeled_files, co_transform_val, 'train')
    ...
    print("Total files in unlabeled  ",len(unlabeled_files))
    ...
    for epoch in range(start_epoch, args.num_epochs+1):
        ...
        if epoch==args.proto_use:
            save_prototypes(args, model.student)
            with open('saved_protos.pkl', 'rb') as f:
...
def save_prototypes(args, model):
    ...
    print(shots_datadir+'nshot_'+nshot+'/city_train_step2.pkl')
    
    dataset_train = cityscapes(city_datadir,train_gt_files, co_transform, 'train')
...
def main(args):
    ...
    if args.eval_type=='train':
        ...
        if args.state:
            savedir_prev_step=savedir_prev_step = 'save/step{}/'.format(current_task)
