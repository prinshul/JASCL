...
class Dimension_reduce(nn.Module):
    def __init__(self):
        super(Dimension_reduce, self).__init__()
        self.conv1 = nn.Conv2d(in_channels=304, out_channels=256, kernel_size=(24, 24))
        self.conv2 = nn.Conv2d(in_channels=256, out_channels=128, kernel_size=(24, 24))
        self.conv3 = nn.Conv2d(in_channels=128, out_channels=64, kernel_size=(24, 24))
        self.conv4 = nn.Conv2d(in_channels=64, out_channels=32, kernel_size=(24, 24))
        self.conv5 = nn.Conv2d(in_channels=32, out_channels=16, kernel_size=(24, 24))
        self.linear1 = nn.Linear(29328,10000)
...
def diverse_samples(args, model):
    ...
    co_transform = MyCoTransform(augment=False,height=args.height, width=args.width)  # 1024)
    co_transform_val = MyCoTransform(augment=False,height=args.height, width=args.width)  # 1024)
    ...
    if data_name == 'BDD':
        print('taking BDD')
        dataset_train = BDD100k(BDD_datadir,gt_train,co_transform,'train',out_files=True,)
        dataset_val = BDD100k(BDD_datadir,gt_val,co_transform_val,'train',out_files=True)
    ...
    ## Reducing dimension for prototype calculation
    mlp = Dimension_reduce()
    mlp = mlp.cuda()
    print(mlp)
    mlp.eval()
    ...
    print("\n\nRunning for train files ")
    with torch.no_grad():
        for step, (images, labels, filenameGt) in enumerate(loader):
            if step%100 == 0:
                print("Reached step ",step)
                print('image size new: ', images.size())
                print('labels size new: ', labels.size())
                print('labels are: ', np.unique(labels.numpy()))
                print('File name : ',filenameGt)
            sys.stdout.flush()    
            inputs = images.cuda()
            targets = labels.cuda()
            
            outputs, fts = model(inputs)
            #print(outputs.shape, fts.shape)
            
            final_proto = mlp(fts)
            proto_dict_train[filenameGt]=final_proto
            
    
    print("Total train files prototype calculated = ",len(proto_dict_train))
    
    with open(savedir+'train_proto.pkl', 'wb') as file: 
        pickle.dump(proto_dict_train, file) 
    
    print("Saved at ",savedir+'train_proto.pkl')
    ## For val files prototype calculation     
    sys.stdout.flush()
    
    print("\n\nRunning for validation files ")    
    with torch.no_grad():
        for step, (images, labels, filenameGt) in enumerate(loader_val):
            if step%100 == 0:
                print("Reached step ",step)
                print('image size new: ', images.size())
                print('labels size new: ', labels.size())
            ...
            final_proto = mlp(fts)
            proto_dict_val[filenameGt]=final_proto
            
    
    print("Total val files prototype calculated = ",len(proto_dict_val))
    
    with open(savedir+'val_proto.pkl', 'wb') as file: 
        pickle.dump(proto_dict_val, file) 
    
    print("Saved at ",savedir+'val_proto.pkl')
    '''
    iouVal = 0
    iouVal, iou_classes = iouEvalVal.getIoU()
    iouStr = getColorEntry(iouVal)+'{:0.2f}'.format(iouVal*100) + '\033[0m'
    ...
    '''
    print("Diverse sampling finished.")
...
def main(args):
    ...
    if args.eval_type=='diverse':
        print("\n\n========== Diverse samples from best model on step1 data ==========")
        ...
        savedir_prev_step='/cil/natural_seg/codes/vanilla_deeplab/save/step1/'
            
        best_checkpoint_file = [fs for fs in os.listdir(savedir_prev_step) if fs.startswith("model_best")][0]
        
        best_checkpoint_path = savedir_prev_step+best_checkpoint_file
        ...
        print('\nBest model is from epoch ',best_checkpoint['epoch'],'\n')
            ...
            #model = model.cuda()
        diverse_samples(args,model_best)
        del model_best
...
if __name__ == '__main__':
    ...
    parser.add_argument('--eval-type', default='diverse')
    ...
    print("\n\nStep 1 : BDD Diverse Samples Calculation\n\n")
