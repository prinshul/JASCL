...
def image_paste_step3_IDD(step1_train, step1_val, train_gt_orig_files, val_gt_orig_files):   
    ...
    step1_data_path = 'data_used/step3/'
    
    step2_save_data = 'data_used/step4/'
    #shutil.rmtree(step2_save_data)
    os.makedirs(step2_save_data,exist_ok=True)
    
    step2_save_path = 'data_used/step4/train/'
    ...
    step1_train.extend(step1_train)
    step1_val.extend(step1_val) 
    ...
    for i in range(0,len(train_gt_orig_files)):
        ...
        ## IDD Datadir
        img_name2 = camvid_datadir+'train/images/'+train_gt_orig_files[i].split("/")[-1].replace("gtFine_labellevel3Ids","leftImg8bit")
        lbl_name2 = camvid_datadir+'train/labels/'+train_gt_orig_files[i].split("/")[-1]
    ...
    for i in range(0,len(val_gt_orig_files)):
        ...
        ## IDD Datadir
        img_name2 = camvid_datadir+'train/images/'+val_gt_orig_files[i].split("/")[-1].replace("gtFine_labellevel3Ids","leftImg8bit")
        lbl_name2 = camvid_datadir+'train/labels/'+val_gt_orig_files[i].split("/")[-1]
...
def train(args, model, start_epoch, best_acc):
    ...
    print('NUM_CLASSES_OLD: ', NUM_CLASSES_OLD)
    ...
    with open(shots_datadir+'nshot_'+nshot+'/cam_unlabel_step4.pkl', 'rb') as file: 
    ...
    curr_path_IDD = 'data_used/step4/' #image_paste_step3_IDD(gaps_train_gt_files[:idd_train_len], gaps_val_gt_files[:idd_val_len], step3_idd_rep_train_files,step3_idd_rep_val_files)
    ...
    dataset_unlabel = Camvid(camvid_datadir,step3_idd_unlabel_files, co_transform_val, 'train')
    
    print("Few Shot per class in Step 4 ",args.nshot)
    print("Total files in Step 4 Camvid train : ",len(step3_idd_rep_train_files))
    print("Total files in Step 4 Camvid val : ",len(step3_idd_rep_val_files))
    print("Total files in Step 4 Camvid unlabeled : ",len(step3_idd_unlabel_files))
    ...
    if  data_name == 'step3_mix':
        ...
        loader = DataLoader(dataset_idd_rep_train, num_workers=args.num_workers,
                        batch_size=args.batch_size, shuffle=True)
        loader_val = DataLoader(dataset_idd_rep_val, num_workers=args.num_workers,
    ...
    print('\n\n\n')
    for name, m in model.student.named_parameters():
    ...
    if args.current_task>0:
        ...
        if args.current_task==1:
            ...
            with open('saved_protos.pkl', 'rb') as f:
                prev_protos=pickle.load(f)
    ...
    alpha=0.99
    print(f"Student teacher alpha = {alpha}")
    ...
    for epoch in range(start_epoch, args.num_epochs+1):
        ...
        for step, (images, labels) in enumerate(loader):
            ...
            for label, feat in prev_protos.items():
                ...
                feat = F.interpolate(feat, size=(3, 3), mode='nearest')
                feat=feat.cuda()
                logits=model.student.decoder.conv_logit(feat)
                
                loss_p_ce=criterion(logits, label)
                L_proto+=loss_p_ce
                del feat, label
            ...
            outputs, _ = model.student(inputs)

            optimizer.zero_grad()
            sup_loss = criterion(outputs, targets[:, 0])
            
            loss = sup_loss + 0.01*L_proto
            
            loss.backward()
            
            
            with torch.no_grad():
                grad_update = (model.student.decoder.conv_logit.mu.weight.grad.clone().detach())**2
                model.student.decoder.conv_logit.grad_update.data = grad_update
                del grad_update
        ...
        if epoch==args.proto_use:
            save_prototypes(args, model)
            with open('saved_protos.pkl', 'rb') as f:
                prototypes=pickle.load(f)
        
        
        epoch_loss_un = []
        if epoch%args.pseudo_label==0 and epoch>=args.proto_use:
            labeled_iter = iter(itertools.cycle(loader))
            print("Unlabeld data self-training")
            for step_un, (images_un, labels_un) in enumerate(loader_unlabel):
                start_time = time.time()
                if args.cuda:
                    inputs_un = images_un.cuda()
                
                ## pseudo mask from student
                outputs_un_st, feat_un_st = model.student(inputs_un)
                out_conv_st = filter_pseudo_labels(outputs_un_st, feat_un_st, prototypes)
                
                
                ## pseudo mask from teacher
                outputs_un_tea, feat_un_tea = model.teacher(inputs_un)
                out_conv_tea = filter_pseudo_labels(outputs_un_tea, feat_un_tea, prototypes)
                
                ## Consistency loss
                mse_loss = F.mse_loss(out_conv_st,out_conv_tea)
                
                del inputs_un, outputs_un_st, feat_un_st, outputs_un_tea, feat_un_tea, out_conv_tea, out_conv_st
                ...
                outputs, feat = model.student(inputs)

                ce_loss = criterion(outputs, targets[:, 0])
                

                #ce_loss = criterion(outputs_un, out_conv[:, 0])  
                #ce_loss = ce_loss.mean() 
                
                total_loss = ce_loss + 0.5*mse_loss 
                

                optimizer.zero_grad()
                total_loss.backward()  
                
                epoch_loss_un.append(total_loss.item())
                
                average = sum(epoch_loss_un) / len(epoch_loss_un)
                if args.steps_loss > 0 and step_un % args.steps_loss == 0:
                    average = sum(epoch_loss_un) / len(epoch_loss_un)
                    print(f'Unlabeled loss: {average:0.4} (epoch: {epoch}, step: {step_un})')
                          
                del inputs, targets, outputs, feat         
            del labeled_iter
        ...
        average_loss_val, val_acc = eval(
            model.student, loader_val, criterion, current_task, args.num_classes, epoch)
        ...
        if (is_best):
            print("best model accuracy : ",epoch)
            test(args, model.student)
...
def test(args, model):
    ...
    test_step3_all = torch.utils.data.ConcatDataset([dataset_test_step1,city_test_step2,
                    IDD_test_step3_rep,cam_test_step4_rep])
    
    
    print("Total test files in Step 4 test : ",len(test_step3_all))
    ...
    print("Total test files in Step 4 Camvid testloader : ",len(cam_test_step4_rep)/args.batch_size)
...
def save_prototypes(args, model):
    print("Saving prototypes for base model")
    global NUM_CLASSES
    NUM_CLASSES = args.num_classes[args.current_task]
    OLD_CLASSES = args.num_classes[args.current_task-1]
    print('NUM_CLASSES: ', NUM_CLASSES)
    ...
    print(shots_datadir+'nshot_'+nshot+'/cam_train_step4.pkl')
    
    dataset_train = Camvid(camvid_datadir,step3_idd_rep_train_files, co_transform, 'train')
    ...
    for cl in range(OLD_CLASSES-1,NUM_CLASSES-1):
        ...
        with torch.no_grad():
            for step, (images, labels) in enumerate(loader):
                ...
                feat_flat = feats.squeeze(0).view(304, -1)
                labels_down = F.interpolate(targets.float(), size=feats.shape[2:], mode='nearest').squeeze(1).long()  
                ...
                protos.append(norm_mean(cls_feats))
                bkg_proto.append(norm_mean(feat_flat[:, ~mask_flat].transpose(0, 1)))  
            ...
            #print(protos.shape, bkg_proto.shape)
            ...
            if protos is not None:
                #wc = wc.unsqueeze(3)
                protos_dict[cl]=protos
                bgkey='bg'+str(cl)
                protos_dict[bgkey]=bkg_proto
            
            sys.stdout.flush()                
        
    for key, val in protos_dict.items():
        print(key, val.shape)
        #print(key, val[0].shape, val[1].shape, val[2].shape)
    
    with open('saved_protos.pkl', 'wb') as f:
...
def main(args):
    ...
    if args.eval_type=='train':
        ...
        if args.model == 'deeplab':
            ...
            student = DeepLab(Xception(output_stride=16), num_classes=args.num_classes[-1])
            teacher = DeepLab(Xception(output_stride=16), num_classes=args.num_classes[-1])
        ...
        if args.state:
            ...
            for k, v in checkpoint_step['state_dict'].items():
                ...
                k = k.replace("module.","")
                if "conv_logit" in k:
                    print(k, v.shape) # model.state_dict()[k].shape
                    print(model.state_dict()[k].shape)
                    model.state_dict()[k][0:nc]=v[0:nc]
                else:
            ...
            student.load_state_dict(load_exp_outc, strict=False)
            teacher.load_state_dict(load_exp_outc, strict=False)
        ...
        total_params_train = sum(p.numel() for p in student.parameters() if p.requires_grad)
        print("Total number of trainable parameters : ",total_params_train)
        
        print("Loading Mean Teacher Model")
        model = MeanTeacherModel(student=student, teacher=teacher)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--proto_use',type=int, default=80)
