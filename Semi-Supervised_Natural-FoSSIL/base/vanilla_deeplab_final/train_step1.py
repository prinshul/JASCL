    ...
    for epoch in range(start_epoch, args.num_epochs+1):
        ...
        for step, (images, labels) in enumerate(loader):
            ...
            if isinstance(outputs, tuple):
                outputs = outputs[0]  # Take only the outputs, ignore features
            else:
                outputs = outputs
...
def save_prototypes(args, model):
    ...
    with open(shots_datadir+'step1_train_files.pkl', 'rb') as file: 
        step1_train=pickle.load(file)
        
    for keys in step1_train:
        print(keys,len(step1_train[keys]))
    ...
    for cl in range(NUM_CLASSES-1):
        ...
        sample_files = random.sample(step1_train[cl], k=min(K,len(step1_train[cl])))
        print(f"Selected {len(sample_files)} samples for class {cl}") 
        dataset_train = BDD100k(BDD_datadir,sample_files, co_transform,'train')
...
def main(args):
    ...
    if args.eval_type=='train':
        ...
        if args.resume:
            ...
            model.load_state_dict(resume_checkpoint['state_dict'], strict=False)
    ...
    if args.eval_type=='test':
        ...
        save_prototypes(args, model_best)
        del model_best
