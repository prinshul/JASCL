...
class SoftTeacher(SemiBaseDetector):
    ...
    def get_pseudo_instances(
        ...
        filtered_data_samples = filter_gt_instances(
            batch_data_samples,
            score_thr=self.semi_train_cfg.pseudo_label_initial_score_thr)

        if self.base_prototypes is not None:
            prototype_sim_thr = 0.7
            roi_extractor = self.teacher.roi_head.bbox_roi_extractor
            bbox_head = self.teacher.roi_head.bbox_head
            
            for i, data_sample in enumerate(filtered_data_samples):
                ...
                bbox_feats = roi_extractor(x[:roi_extractor.num_inputs], rois)
                
               
                bbox_feats_flat = bbox_feats.view(bbox_feats.size(0), -1)
                instance_features = bbox_feats_flat
                for fc in bbox_head.shared_fcs:
                    instance_features = fc(instance_features)
                
                if not self.debug_printed:
                    print("\n--- soft_teacher.py: DEBUG SHAPES ---")
                    print(f"Shape after RoI pooling (bbox_feats): {bbox_feats.shape}")
                    ...
                    print("--- This confirms features are 1024-dim before comparison. ---\n")
                    self.debug_printed = True

                keep_mask = torch.ones_like(gt_labels, dtype=torch.bool)
                
                for j in range(len(gt_instances)):
                    label = gt_labels[j].item()
                    
                    if label in self.base_prototypes:
                        instance_feat = F.normalize(instance_features[j], p=2, dim=0)
                        proto_feat = F.normalize(self.base_prototypes[label], p=2, dim=0)
                        
                        similarity = torch.dot(instance_feat, proto_feat)
                        
                        if similarity < prototype_sim_thr:
                            keep_mask[j] = False
                
                data_sample.gt_instances = gt_instances[keep_mask]
        
        reg_uncs_list = self.compute_uncertainty_with_aug(
            x, filtered_data_samples)

        for data_samples, reg_uncs in zip(filtered_data_samples, reg_uncs_list):
        ...
        for data_samples in filtered_data_samples:
        ...
        return filtered_data_samples, batch_info
    ...
    def rcnn_cls_loss_by_pseudo_instances(self, x: Tuple[Tensor],
        ...
        if 'loss_cls' in losses:
            ...
            losses['loss_cls'] = torch.tensor(0.0, device=torch.device("cuda"))
        return losses
